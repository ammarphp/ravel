...
> `docs/development/change-registry.md` (open/fixed defects). Created 2026-07-06 (charter §4.2).
...
(`docs/development/history/mission-and-plan.md` STATUS 2026-07-06).
...
1. **Shape/template-fit statistics** — NO LONGER a blanket refusal (supervisor decision 2026-07-07,
   `docs/development/decisions/shape-fit.md` Option B). Analyses whose result is a binned shape/bump/template
   ...
   surveyed population (`docs/development/status.md` census) that the counting machinery alone could not.
...
- Claim integrity: every headline/served claim in the SHIPPED docs (README,
  `docs/workflow/reference/*`, PRODUCT-CONTRACT) maps to a shipped, sha256-checksummed artifact via
  `evidence/manifest.json`; `scripts/maintenance/export-distribution.sh` runs `check_evidence.py --check --stage` and
...
is served/partial/unbuilt; consumed by `audit.py`→`docs/development/audit.md`); `docs/development/history/mission-and-plan.md` states INTENT;
`docs/development/roadmap.md` states SEQUENCING. Every prose readiness/served/refusal claim in
