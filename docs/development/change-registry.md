...
- **Status:** OPEN — fresh nominal 150/140 and 100/98 mass points have completed all twelve native stages and six roots. The independent 20k/40k 150/140 pool, six-state control and lower-parton control are complete. The pooled nominal limits agree closely with the reference, while the lower-cut limits differ substantially; neither closes acceptance or radiation modeling. Exact original-LHA identity now supports a complete 20k hard-parton partition. The fresh100/98 result remains −11.81%/−17.09% from the observed/median reference with sparse MC. Eight of 52 historical-template points have checked roots, with 44 unrun and original failures retained. No full fresh-grid, truth-acceptance, coverage or physics-closure claim.
- **Verify:** At the initial closure checkpoint, the complete source suite passed 1,475 tests with 12 skips; the relevant native/ROOT suite passes 230 and the freshly built-wheel CLI suite passes all 40, including both isolated wheel checks. Fresh native/statistical audits replaced the stale-current-audit failure with actual revalidation, preserving the original audit bytes. A separate relevant ROOT suite passed 225 cases. Later checks include 199 integration passes with ten ROOT skips, 38 integrity passes, 50 publication/evidence passes, 59 origin tests and independently repeated derivative/reader controls. The independent 20k conditional-likelihood control matches saved CLs values within 3.975e−8 at two tested roots. The full benchmark retains two genuine missing-YODA provenance failures and unresolved acceptance cases. Final release verification remains pending after the last relevant changes.
...
- **2026-09-06 execution/procedure follow-up:** Repair bounded cleanup and terminal timeout recording; enforce the registered 1.26-million ceiling under the original 1.28-million allocation; isolate/fix Git-hook installation tests. All 64 supervisor checks, 38 admission/storage checks plus 14 independent controls, and 75 hook/fixture checks passed. V5 preserves all common physics/inference bytes. Source-backed model-interface review replaces a universal MSOFT override claim; active merging guidance no longer exempts leptonic recoil-sensitive searches or treats a historical 5% rate comparison as a universal law. That follow-up source suite passed 1,533 cases with 12 optional skips. Its public export passed 1,524 tests with 21 optional skips. The later cut-control bundle was published at fcd53f90 with seven successful CI jobs; newer implementation/evidence verification is recorded separately. See [the follow-up record](history/2026-09-06-rrr-execution-followup.md).


## CR-156 · 2026-09-06 · exact original-event identity and activated native inputs
- **What:** Add optional original-LHA sidecars and complete new-output verification, descriptor-bound plain/gzip source custody, activated LHAPDF library/PDF decisions, per-call validation hash sharing with final mutation checks, and canonical child-plan accounting. Add a source-bound fresh100/98 and hard-parton partition evidence bundle with complete missing/sparse populations.
- **Why:** Actual execution exposed ambiguous child-plan matching, repeated ancestor hashing, loss of original hard-event identity after valid shower transformations, and fragile native linker context. Independent review additionally reproduced path/ledger replacement races and removable evidence-role gaps.
- **Where-embedded:** `src/ravel/physics/lhe_provenance.py`, `native_lhapdf.py`, `native_event_io.py`, `native_pipeline.py`; `native/src/pythia_shower.cc`; `src/ravel/workflow/validation_io.py`, `execution.py`, `campaign_budget.py`; corresponding adversarial tests and the publication audit dispatcher. Read [the implementation and evidence record](history/2026-09-06-native-provenance-and-fidelity.md).
- **Status:** Implemented and independently reviewed. The complete staged own-wheel suite passes 1,980 tests with 12 optional skips. The isolated real 100-event compile/plain/gzip/validation exercise and its independent output review are complete, with exact original-content joins and complete decoded equality. All 96 populated dry-plan input snapshots across eight option/encoding combinations also pass. Original physics runtimes, historical audits and failed attempts are preserved. Public export and remote CI remain separate verification steps. This bounded engineering follow-up does not close CR-155's physics obligations.

## CR-157 · 2026-09-06 · complete three-anchor evidence and explicit override diagnostics
- **What:** Publish the completed fresh 50/45 point beside 100/98 and the original-exposure 150/140 pool, retaining all 18 limits, 114 channel moments and six reconstructed fractions. Derive the README's new limit and nominal-point coverage headlines from the verified bundle. Identify forbidden native LHAPDF environment overrides by field name without disclosing values or changing refusal rules.
- **Why:** Agreement at one nominal mass point conceals opposite residual signs elsewhere, sparse-bin precision and missing reference definitions. A generic environment error also hid the fields responsible for a failed PDF activation. Neither issue is solved by changing normalization to fit the reference or weakening an execution guard.
- **Where-embedded:** `evidence/audits/2026-09-06-rrr-fresh-anchors/`, `scripts/claims_check.py`, `scripts/check_publication.py`, `tests/unit/test_publication_scopes.py`, `src/ravel/physics/native_lhapdf.py`, its unit tests, the current fidelity-audit bridge and explicit CI evidence checks.
- **Status:** Three-point source/scalar review closed: 90 selected small original roles, 114 direct original-model channel comparisons, 394 Decimal calculations, 15 independently parsed reference coordinates and 72 portable checks. The five new public-headline controls pass within the 25-test publication-scope suite. The environment-message change passes 180 relevant tests and 295 independent refusal/privacy cases. These populations overlap with their broader suites and must not be summed. Integration, export and remote CI are separate release checks. Three of 52 nominal points are complete; truth acceptance, generator-cut robustness, detector/theory uncertainties, sparse-template precision, coverage and full reproduction remain open.
