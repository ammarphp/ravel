# Historical capability roadmap

The July planning record is retained below. Current implementation and research priorities are in [the roadmap](../roadmap.md).
...
by `benchmarks/capabilities.json` (viewed via `audit.py` R9 / `AUDIT.md` / the
...
(CR-001/002/003 fixed; CR-007…CR-014 embedded; 7 new + 6 fixed skills; `docs/reference/scope.md`; the
...
`docs/workflow/README.md` scopes the pipeline to "analyses that have **either** a Rivet routine **or**
...
| P4 | Dijet+photon 2408.00049, Fig 5 + wide widths | Ran as the 2026-06-21 generality audit: **correctly BLOCKED (statistical-paradigm)** — background shape-fit, the ~40% boundary (8/20 in the pre-registered census, `docs/research/reviews/generality.md`). The open fork is a *decision*, not a mystery: keep the named refusal vs build a scoped binned-fit engine | BOUNDARY — DECISION NEEDED (G2b) |
...
Launch: `Development session: finish docs/development/history/operability-charter.md (P4 live evals, P5), then
...
- **W2c Limitations triage** (§6). Launch: `Development session: triage docs/reference/limitations.md
...
- **Output contract:** `docs/research/reviews/capability-census.md` — the matrix, category mass,
