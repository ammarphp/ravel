...
`benchmarks/cases.json` registers each: `{analysis_id, run_dir, routine, grid, m_parent,
...
- `benchmarks/cases.json` — the registry.
- `benchmarks/run_benchmark.py` — loads `cases.json`, runs the three metrics per case (calling
  the existing helpers), prints a tier table, writes `benchmarks/results.json`, and exits
...
- `docs/validation/benchmark-guide.md` — what it measures, how to run it, and the **baseline scores**
