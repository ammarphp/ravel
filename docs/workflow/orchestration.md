...
(For the multi-**session** build plan — which session to run next — see `docs/development/roadmap.md`;
this file is the per-run *pipeline* map, that one is the per-project *session* map.)

> **The pipeline is implemented and certified in `docs/workflow/`.** Start at `docs/workflow/README.md` (9 steps; 8 = scan→contour, 9 = the mandatory verification panel).
> It runs **natively** (conda, no containers) on the **Rivet path** AND — by default — on the
> **SimpleAnalysis path** (the VM-free native backend, `docs/workflow/reference/native-pipeline.md`;
> the podman+mapyde container is the legacy fallback for analyses with no native port). Pick the
> path by what the analysis provides (`docs/workflow/checklists/choosing-routine.md`).
...
2. Open `docs/workflow/README.md` and follow the 9 steps; **load each `docs/workflow/steps/NN-*.md` on demand** (one at a
   time) to keep context small.
3. Carry only the **interface artifact** (the output file) between steps — not the whole prior context.
4. Record environment changes in `docs/development/history/environment-changes.md` and card/data
   changes in `docs/development/history/data-and-card-changes.md`; record run outcomes in `RESULT.md`.
...
## Conventions
Before authoring or editing any instruction file, read `docs/development/documentation.md` (keeps
