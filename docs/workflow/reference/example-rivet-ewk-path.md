...
A filled-in shape of `docs/workflow/README.md` for a leptonic electroweak search whose Rivet routine is
...
| Step | What you do |
|---|---|
| 2 routine | `routine_fetch.py --query "[code/Inspire]"` → the Rivet id; **check `--show-analysis` for `SINGLEWEIGHT`/`Reentrant: false`** (`docs/workflow/steps/04-analyze.md`) |
| 2 model | an electroweakino simplified model (wino C1/N2 + bino LSP → WZ); card per the **EWKino section** of `docs/workflow/checklists/model-cards.md` — record the actual model mass/mixing interface, **include `MODSEL`**, and declare C1→WN1 / N2→ZN1 decays |
| 3 generate | `p p > x1+ n2` / `add p p > x1- n2` at the routine's √s; choose radiation and matching from the published simulation and recoil-sensitive selection. **Verify the LHE particle mass == the intended mass** and that the shower makes leptons, *before* trusting the run (wrong card/model interface) |
...
wrong effective card/model inputs and empty decay tables (wrong mass / no decays); `--skip-weights` for
