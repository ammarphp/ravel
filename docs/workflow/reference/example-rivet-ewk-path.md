...
| 2 model | an electroweakino simplified model (wino C1/N2 + bino LSP → WZ); card per the **EWKino section** of `docs/workflow/checklists/model-cards.md` — set `MSOFT` M1/M2 (not just `MASS`), decouple μ, **include `MODSEL`**, force C1→WN1 / N2→ZN1 |
