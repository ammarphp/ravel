...
Run the following from the repository root in Bash and retain `RAVEL_REPO` after changing directories:
```bash
source native/scripts/paths.sh
CONDA="$RAVEL_NATIVE_BUILD/tools/miniforge3/bin/conda"
```

This branch produces the analysis inputs; the 95% CL limit itself is computed in `docs/workflow/steps/07-exclude.md`.
...
```bash
$CONDA run -n rivet python "$RAVEL_REPO/scripts/run.py" ravel.physics.pyhf_exclude likelihood \
  --bkg <bkgonly>.json --patch <analysis>_patch.json --out pyhf_exclusion
```
`exclusion.json` gives the observed/expected µ₉₅; **µ₉₅ < 1 ⇒ excluded**.

**Record the run** (fill `<RUN>/RESULT.md` per `docs/workflow/run-directory.md`): per-stage PASS/timing, config,
...
**Next:** `docs/workflow/steps/07-exclude.md` → report the path, µ₉₅, and verdict.
