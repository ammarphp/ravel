...
Read with `docs/workflow/steps/09-verify.md`. MANDATORY before any result is delivered: the drafted final
...
- **Any mid-run change of protocol, tolerance, selection, or instruction MUST be logged in the run's
  ...
  touches. The ledger's check-in is defined in `docs/workflow/checklists/check-ins.md`. An unlogged change
...
Run `src/ravel/validation/validate_run_state.py --rundir <rundir>` first (lifecycle: required
stages present + invariants incl. R5/pairing/ladder-order/certify-before-limit), then
`src/ravel/validation/verify_pack.py <rundir>` (artifact integrity); paste both reports here.
Then, by hand:
...
- [ ] **Verification ladder present + consistent** — `VERIFICATION-LADDER.md` exists
      (`docs/workflow/checklists/verification-ladder.md`), every rung has exactly one status, every RESULT.md
...
- [ ] **Every displayed figure has a caption** (`what_it_shows` populated in `figures.json`; the
      check-in shows it). `criteria_pass` recorded per `docs/workflow/checklists/plot-criteria.md`.
- [ ] **Figure contract fulfilled or WARNed** — declared targets carry a `generated_counterpart`
      (`docs/workflow/checklists/figure-contract.md`); unfulfilled targets are explicitly WARNed, not omitted.
...
process failures — `docs/reference/failure-modes.md`, walking each catalogued class against this
run — and the domain trap catalogue — `docs/workflow/checklists/physics-traps.md` T1–T12, RE-RUNNING the sweep
independently (a mismatch with the run's recorded `traps_checked/traps_hit` is itself a finding).
The run's own P8 kill-the-result attacks (`docs/workflow/checklists/judgment-protocols.md`) are seeds, not
