...
when_to_use: after changing any package or native tool, a workflow step/checklist, or a process — before reporting the change complete
...
- New/changed `src/ravel/` or `native/` tool → documented in the relevant `docs/workflow/steps/NN-*.md`
  and/or `docs/workflow/checklists/*.md` (the invocation, the flags, when to use it).
...
- New workflow step/gate → wired into `docs/workflow/README.md` (the step table) and cross-referenced from
  the adjacent steps.
- Quick audit: `grep -rIE '<tool-or-flag>' docs/workflow/ .claude/` — **0 hits = unembedded = not done.**
- Update `DIRECTORY.md` for any new file (or run the `directory-keeper` skill).
- **Skill edited?** Re-mirror: `python3 scripts/maintenance/sync_skills.py` (single source =
  `.claude/skills/`; the surface gate fails on drift).

## 1b. REGISTER (binding — charter §7: every change lands in the registry)
`docs/development/change-registry.md` gets its entry in the SAME commit: a new `CR-NNN` (ID · date ·
...
## 2. Keep it shippable
- Run the surface gate: `python3 scripts/run.py ravel.validation.check_agent_surface` — it asserts
  ...
  - The surface gate is now ALSO a git **pre-commit hook** (D16/G20) — install it once per
    clone/worktree with `bash scripts/maintenance/install-git-hooks.sh`. It writes a
...
- Run the distribution-hygiene grep (`docs/development/distribution.md`); it must stay clean (no run-specific
...
- If infra in the benchmark path changed (`pyhf_exclude.py`, `validate_cutflow.py`, `cases.json`), run
  `python3 scripts/run.py ravel.validation.benchmark --full` and confirm `GATE: OK`.
  Routine runs write to a gitignored scratch output by default; use `--work-dir` and `--out`
  for a separate writable destination. Inspect every reported BREACH and its logs. Baseline updates
  require an explicit `--update-baseline` and a justified review; never refresh a baseline to pass.
...
| Thought | Reality |
|---|---|
| "The code runs — the change is done" | Done is the bold line above. 0 grep hits in `docs/workflow/` = unembedded = not done (the classic new-flag gap). |
...
- Benchmark BREACH after touching gated infra → diagnose the failed case and retain its logs.
  A repeat run can test a suspected transient cause; an unexplained breach blocks the commit.
