# hep-agentic-pipeline

**An agentic system that turns natural-language physics questions into validated collider
simulations and statistical exclusion limits.**
...
![tests](https://img.shields.io/badge/tests-344%20collected-blue)
...
> The numbers in this document are CI-verified against their generating artifacts:
> [`scripts/claims_check.py`](scripts/claims_check.py) fails the build if any figure here drifts
...
> artifacts by sha256 ([EVIDENCE.md](EVIDENCE.md)).

## Results at a glance

| Result | Number | Evidence |
|---|---|---|
| Published-analysis benchmarks | <!-- claim:benchmarks_reproduced -->7 within 8.6%<!-- /claim --> of published limits (7 reproductions across 4 distinct searches; best 0.2%) | [docs/validation/](docs/validation/README.md) |
...
| Flagship reproduction | full 52-point ATLAS mass-plane scan; <!-- claim:fig3_residual -->24.9% median same-basis residual<!-- /claim --> vs the published contour | [scan record](trial-runs/sleptonscan_fig3_SCAN/RESULT.md) |
| Verification surface | <!-- claim:tests_collected -->344<!-- /claim --> tests · <!-- claim:test_modules -->59<!-- /claim --> test modules · <!-- claim:adversarial_gate_cases -->30<!-- /claim --> adversarial gate cases | CI runs of this repo |
...
| Scale exercised | <!-- claim:scan_scale -->2.08M events across two 52-point scans<!-- /claim -->; <!-- claim:exclusion_computations -->112<!-- /claim --> per-point pyhf exclusion artifacts | [results/manifest.json](results/manifest.json) |
...
layers — proving the control plane without MadGraph.

```bash
git clone https://github.com/ammarphp/ravel.git && cd ravel
pip install -r requirements-replay.txt
make replay
```

Expected output: the fast benchmark case's row — reproduced vs published limit, verdict
...
What is genuinely novel is the governance: typed contracts between stages, machine-checked
...
automated. It exists because recasting published searches against new models is currently
manual, slow, and error-prone — this repo is an experiment in making it a governed,
reproducible loop.
...
## Validated against published analyses: 7 benchmark reproductions across 4 searches

Seven known-answer benchmark reproductions spanning four distinct published searches (the
squark/gluino cases share one analysis at different model points and generator settings):
ground truth transcribed from the published analysis,
the reproduction re-fit through the real pyhf layer on every gate run. Δ compares our observed
s95 against the published value; per-case pages carry config, seed, signal region, stability
check, and the regeneration command.
...
24.9% median residual, decomposed into its causes (fast-simulation floor, LO statistics; the PDF
term measured at +6.5% by a paired 52-point rescan) in the
...
| `trial-runs/_infrastructure/` | the toolchain: orchestrators, gates, statistics, renderers (56 modules) |
| `framework/` | quality bar: benchmark gate, evidence manifests, audits, failure catalogue, registries |
| `framework/tests/` + `framework/spine_sim/` | the 344-test suite and the 30-case adversarial gate board |
...
- Detector simulation is Delphes-level fast-sim; the intrinsic acceptance floor (~10–20% on
  compressed spectra) dominates the flagship residual. No Geant4.
- The native SimpleAnalysis backend covers 3 bit-for-bit-validated routines plus a declarative
  ...
  emulated) container path, and the two newest ports serve full results only after their
  acceptance certifications land. The Rivet path is native everywhere.
...
*Public snapshot of an active research repository; the development history lives in the private
working repo and is squashed into sanitized export commits here.*
