...
Run from anywhere: `python scripts/audit.py`.
...
`--write [--out PATH]` to regenerate the report on disk (default PATH is docs/development/audit.md).
...
from ravel.evidence_layout import resolve, load_registry
...
def _run_dirs():
    ...
    if os.path.isfile(registry):
        for collection in load_registry(ROOT)["collections"]:
            candidate = str(resolve(ROOT, collection["source"]))
...
def c_validation():
    certs = glob.glob(p("docs", "validation", "studies", "*.md")) + glob.glob(p("evidence", "validation", "studies", "*.json"))
    runs = trial_runs()
    routines = set()
    for r in runs:
    ...
    return ("R1", "Physics validation (tiered cutflow verdict)", score, status_from(score),
            ...
            [os.path.relpath(c, ROOT) for c in certs] or ["validation studies are empty"])
...
def c_completedata():
    ...
    route = "full-table route" in read("docs/workflow/checklists/data-acquisition.md").lower() \
...
def c_complexity():
    ...
    note = exists("docs", "validation", "studies", "complex.md") or exists("docs", "workflow", "checklists", "complex-analysis.md")
...
def c_fidelity():
    verdicts = glob.glob(p("trial-runs", "20*", "**", "*fidelity*"), recursive=True) \
               + glob.glob(p("docs", "validation", "studies", "*fidelity*"))
...
def c_crosscheck():
    cc = glob.glob(p("docs", "validation", "studies", "*crosscheck*.md")) \
         + glob.glob(p("evidence", "crosschecks", "*.smodels")) \
         + glob.glob(p("trial-runs", "20*", "outputs", "**", "*smodels*"), recursive=True) \
         + glob.glob(p("docs", "validation", "studies", "*crosscheck*"))
...
def c_docs():
    need = ["docs/workflow/README.md", "docs/workflow/session-guide.md", "README.md", "DIRECTORY.md",
            "docs/guides/pipeline-guide.pdf", "docs/workflow/checklists/validation.md"]
...
def c_deps():
    manifest = exists("docs", "reference", "environment.md") or glob.glob(p("framework", "*environment*.yml")) \
    ...
    return ("R8", "Dependency / version manifest", score, status_from(score),
            ...
            ["docs/reference/environment.md"] if manifest else ["no pinned environment manifest yet"])

def c_limits():
    t = read("docs/reference/limitations.md")
    ...
    return ("R8", "Known-limitations registry", score, status_from(score),
            ...
            ["docs/reference/limitations.md"] if t else ["missing"])

def c_plotcriteria():
    # publication-grade plotting: the criteria checklist + at least one mplhep overlay produced
    crit = exists("docs", "workflow", "checklists", "plot-criteria.md")
    overlays = glob.glob(p("trial-runs", "20*", "plots", "**", "*overlay*.p*"), recursive=True)
    mplhep = "mplhep" in read("src/ravel/plotting/overlay_on_data.py")
...
def c_merging():
    ...
    doc = exists("docs", "workflow", "checklists", "merging.md")
    applied = []
    for r in trial_runs():
        t = read(os.path.relpath(os.path.join(r, "RESULT.md"), ROOT)).lower()
...
def c_hygiene():
    ...
    dist = exists("docs", "development", "distribution.md")
    pat = re.compile(r"gluino-pair|squark-pair|slepton_200|2026-06")
    refs = []
    scan = glob.glob(p("docs", "workflow", "**", "*.md"), recursive=True) + [p("README.md"), p("docs", "workflow", "session-guide.md")]
    for f in scan:
...
def _gate_verdict(gate, _cache={}):
    ...
    if kind == "artifact":
        ...
        if not art:
            ...
            try:
                path = p(art) if os.path.isabs(art) else str(resolve(ROOT, art))
            except (OSError, ValueError) as exc:
                return False, f"artifact invalid: {exc}"
    ...
    if kind == "selftest":
        ...
        if not ref:
            ...
            try:
                command = ([sys.executable, p("scripts/run.py"), ref[4:-3].replace("/", "."), "--selftest"]
                           if ref.startswith("src/ravel/") and ref.endswith(".py")
                           else [sys.executable, p(ref), "--selftest"])
...
def render():
    ...
    lines = [f"# AUDIT — pipeline readiness  ·  {readiness}%", "",
             f"_Generated by `scripts/audit.py` ({stamp}). Requirements R1–R8 are defined in `STATUS.md`;_",
...
def main():
    ...
    ap.add_argument("--out", default=None,
                     help="write path when --write is given (default: docs/development/audit.md)")
    args = ap.parse_args()

    text = render()
    if args.write:
        out = args.out if args.out else p("docs", "development", "audit.md")
