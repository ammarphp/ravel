#!/usr/bin/env python3
"""scripts/verify_fixes.py — the ONE aggregated `verify:` board for the audit-and-fix CRs.

The hybrid verification layer (Task 9): the durable green is a DETERMINISTIC command per
audit-and-fix item (CR-030..CR-043 in `docs/development/change-registry.md`), not a hand-maintained
...
Usage:
    python3 scripts/verify_fixes.py             # human board, exit 0/1
    python3 scripts/verify_fixes.py --json       # machine board on stdout, same exit code
    python3 scripts/verify_fixes.py --registry P # override CHANGES-REGISTRY.md path (tests)
"""
...
DEFAULT_REGISTRY = REPO_ROOT / "docs" / "development" / "change-registry.md"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--registry", default=str(DEFAULT_REGISTRY),
                         help="override docs/development/change-registry.md path (test hook)")
    return parser
