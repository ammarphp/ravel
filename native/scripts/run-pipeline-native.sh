...
LAUNCHER="$REPO/scripts/run.py"
MAPYDE_SHARE="$TOOLS/miniforge3/envs/pipeline/share/mapyde"

# SimpleAnalysis is the NATIVE Python+RestFrames port (ravel.physics.native_simpleanalysis) -- no x86 container, no ATLAS AnalysisBase. It reproduces
...
run_stage() { # <name> <command...>  -> logs to logs/<name>.log, records PASS/FAIL+secs
  ...
  if [ "${STAGE_SUPERVISED:-1}" = "1" ] && [ -f "$LAUNCHER" ] && command -v python3 >/dev/null 2>&1; then
    python3 "$LAUNCHER" ravel.workflow.stage_supervisor --stage "$name" --rundir "$RUNDIR" \
...
run_stage lhe_check "$CONDA" run --live-stream -n rivet python "$LAUNCHER" ravel.validation.lhe_check "$LHE" \
    ${GATE_ARGS[@]+"${GATE_ARGS[@]}"}

# ---- 2. pythia shower (rivet env -> HepMC3) ----------------------------------
HEPMC="output/madgraph/tag_1_pythia8_events.hepmc"
mkdir -p "$(dirname "$HEPMC")"
run_stage pythia "$CONDA" run --live-stream -n rivet "$RAVEL_NATIVE_BIN/pythia_shower" \
...
run_stage analysis "$CONDA" run --live-stream -n recast python "$LAUNCHER" ravel.physics.delphes2sa_native \
...
run_stage simpleanalysis "$CONDA" run --live-stream -n rivet python "$LAUNCHER" ravel.physics.native_simpleanalysis \
...
run_stage sa2json "$CONDA" run --live-stream -n rivet python "$LAUNCHER" ravel.physics.sa2json_native \
...
run_stage pyhf "$CONDA" run --live-stream -n rivet python "$LAUNCHER" ravel.physics.pyhf_exclude likelihood \
