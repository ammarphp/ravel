...
THE CORRECTED MENTAL MODEL (read docs/workflow/steps/08-scan.md). The mapyde paper (arXiv:2306.11055) never
...
def cmd_plan(args):
    ...
    print(f"  python src/ravel/workflow/scan_orchestrator.py launch "
          f"{os.path.relpath(scandir, REPO)} --backend native --max 4 --go")
    print(f"Then assemble when points are done:")
    print(f"  python src/ravel/workflow/scan_orchestrator.py assemble "
...
def apply_nlo_renorm(rows, process, man):
    ...
    from ravel.physics import nlo_xsec
    try:
    ...
    return {"process": process, "flat_k_replaced": flat_k,
            ...
            "k_denominator": "src/ravel/data/cross_sections/slepton_selL_lo_cteq6l1.json "
...
def cmd_rebase(args):
    ...
    from ravel.physics import nlo_xsec
    def table(name):
        p = str(package_data_path("cross_sections", name))
