...
LIKELIHOOD mode (the second half of the spec, `docs/workflow/reference/projection-replane.md`):
...
CONDA = str(native_build_root() / "tools/miniforge3/bin/conda")


def _python():
    return [CONDA, "run", "-n", "rivet", "python"] if os.path.isfile(CONDA) else [sys.executable]
...
def run_engine(srs_path, outdir, sigma_scale=None, combined=False):
    cmd = module_command("ravel.physics.pyhf_exclude", python=_python()) + [
...
def _selftest():
    import tempfile
    from ..resources import FAST_RUN, resource_root
    from ..evidence_layout import resolve
    srs_path = str(resolve(resource_root(), FAST_RUN + "/outputs/sr_yields_fitted.json"))
