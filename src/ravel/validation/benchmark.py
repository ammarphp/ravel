...
subprocess; this script needs only python3. Run from anywhere:
    ...
    python3 scripts/run.py ravel.validation.benchmark --fast     # one case, minutes
    python3 scripts/run.py ravel.validation.benchmark --full     # all cases
    python3 scripts/run.py ravel.validation.benchmark --case ID [--case ID2]
"""
if not __package__:
    import sys as _sys
...
from ..paths import module_command, native_build_root, repository_root
from ..resources import resource_root
from ..evidence_layout import resolve

REPO = resource_root()
BENCH = REPO / "benchmarks"
# Installed assets stay read-only even when the lower-level benchmark CLI is used.
WORK = ((repository_root() / "benchmarks") if repository_root() else Path.cwd()) / ".work"
DEFAULT_OUT = str(WORK / "results.latest.json")
TRACKED_OUT = str(BENCH / "results.json")
CONDA = native_build_root() / "tools/miniforge3/bin/conda"
ENV = "rivet"
...
REFETCH_HINT = ("missing HEPData tables — refetch with: <conda> run -n reinterp python "
                "scripts/run.py ravel.workflow.hepdata_fetch (see docs/validation/benchmark-guide.md)")
...
def run_pyhf(case, wdir):
    out = wdir / "pyhf"
    cmd = module_command("ravel.physics.pyhf_exclude", python=_py_invoke()) + ["counting",
           "--srs", resolve(REPO, case["inputs"]["sr_yields"]), "--out", out,
...
def run_cert(case, wdir, fresh_exclusion):
    ...
    if e["engine"] == "validate_cutflow":
        cmd = module_command("ravel.validation.validate_cutflow", python=_py_invoke()) + [
               "--signal", resolve(REPO, case["inputs"]["yoda"]), "--routine", case["routine"],
               "--sigma-pb", case["sigma_lo_pb"], "--lumi-fb", case["lumi_fb"],
               "--tables-dir", resolve(REPO, e["tables_dir"]), "--grid", e["grid"],
        ...
        cmd = _py_invoke() + [resolve(REPO, e["script"]),
               "--axe", resolve(REPO, e["axe_json"]), "--exclusion", fresh_exclusion,
               "--sr-yields", resolve(REPO, case["inputs"]["sr_yields"]),
               "--driving-tol", e["driving_tol"], "--contributing-tol", e["contributing_tol"],
...
def score_provenance(case):
    failures = []
    run_dir = resolve(REPO, case["run_dir"])
...
def build_parser():
    ...
    ap.add_argument("--out", default=DEFAULT_OUT,
                    help="results path — defaults to a gitignored scratch file so a routine/"
                         ...
                         "benchmarks/results.json) to refresh the committed baseline")
    ap.add_argument("--update-baseline", action="store_true",
                    help="write the TRACKED benchmarks/results.json baseline instead "
