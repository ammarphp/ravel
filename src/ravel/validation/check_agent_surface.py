...
Checks (each PASS/FAIL, exit 1 on any FAIL):
  fork        CLAUDE.md + AGENTS.md agree: physicist route -> docs/workflow/start.md; dev read
              order = DIRECTORY.md -> docs/development/status.md -> docs/development/history/mission-and-plan.md
  ...
  stepcount   'N steps' claims across routing docs == the actual count of docs/workflow/steps/*.md
  hygiene     no dev trial-run tokens in docs/workflow/ + .claude/skills + .claude/rules + README
...
from ravel.paths import require_repository_root, repository_root
...
DEV_READ_ORDER = ("DIRECTORY.md", "docs/development/status.md", "docs/development/history/mission-and-plan.md")
ROUTING_DOCS = ("CLAUDE.md", "AGENTS.md", "README.md", "docs/reference/scope.md")
STEPCOUNT_DOCS = ROUTING_DOCS + ("ORCHESTRATION.md", "docs/workflow/orchestration.md",
                                 "docs/development/status.md", "docs/workflow/README.md", "DIRECTORY.md")
...
def _runtime_or_pattern(tok, root):
    ...
    if tok.endswith("/") and "/" not in t:                  # run-dir convention: `plots/`, `inputs/`
        return True
    if t.startswith("trial-runs/") and any(ch in t for ch in "*?"):
        return True  # generated run-layout conventions, not a shipped archive path
...
def _resolve(tok, base_dirs, root=None):
    ...
    or, for a bare filename mention only, its basename exists in the tree."""
    for var in _expand_braces(tok.rstrip("/")):
        for base in base_dirs:
            ...
            if any(ch in var for ch in "*?") and globmod.glob(cand):
                return True
    if root is not None and "/" not in tok.rstrip("/"):
...
def check_refs(root, files, label):
    ...
    for f in files:
        ...
        # also accept one level up (docs/workflow/analysis-simpleanalysis uses ../)
...
def check_fork(root):
    ...
    for name, txt in docs.items():
        if "docs/workflow/start.md" not in txt:
            errs.append(f"{name}: physicist route does not name docs/workflow/start.md")
...
def check_dirmap(root):
    """DIRECTORY.md is SECTIONED: '## `docs/workflow/` ...' headers set the base its rows are
    ...
    for line in open(p, encoding="utf-8"):
        ...
        if cell.lower() in ("path", "---", ":---") or set(cell) <= {"-", ":", " "}:
            continue
        toks = [t for t in REF_RE.findall(cell)
                if _is_pathlike(t) or os.path.exists(os.path.join(root, base, t))]
        if not toks:
    ...
    for e in sorted(os.listdir(root)):
        if e.startswith(".") or e in ("__pycache__", "py.py", "parser.out", "MG5_debug", "logs"):  # gitignored scratch
            continue
        if e not in mapped:
...
def check_readiness(root):
    ...
    audit = os.path.join(root, "docs", "development", "audit.md")
    if not os.path.exists(audit):
        return ["docs/development/audit.md missing"]
    m = re.search(r"readiness\s*·\s*(\d+)%", open(audit, encoding="utf-8").read())
    if not m:
        return ["docs/development/audit.md carries no 'readiness · N%' headline"]
    truth = m.group(1)
    ...
    errs = []
    for rel in ("README.md", "docs/development/status.md", "DIRECTORY.md", "CLAUDE.md",
        ...
        for rx in claim_res:
            for got in rx.findall(txt):
                if got != truth:
                    errs.append(f"{rel}: claims {got}% but docs/development/audit.md says {truth}%")
    return errs
...
# committed AUDIT.md — same technique as scripts/gen_status.py), then asserts (1) the
...
STATEFRESH_FILES = (
    "docs/development/status.md", "README.md", "docs/reference/limitations.md", "CLAUDE.md",
    "DIRECTORY.md", "docs/reference/scope.md", "docs/workflow/steps/04-analyze.md",
    "docs/workflow/reference/projection-replane.md", "docs/workflow/reference/effmap-folding.md",
    "docs/development/decisions/shape-fit.md",
)
...
STATEFRESH_ROADMAP_FILES = ("docs/development/roadmap.md", "docs/development/history/mission-and-plan.md")
...
# OPPOSITE of the forbidden claim and must not fire — e.g. docs/reference/scope.md's own §6 lede.
...
def _matrix_truths(root):
    """The matrix-derived truth, computed LIVE — mirrors scripts/gen_status.py's own
    technique exactly (import scripts/audit.py for readiness/R9; read capability-matrix.json
    ...
    fw = os.path.join(root, "scripts")
    matrix_path = os.path.join(root, "benchmarks", "capabilities.json")
    ...
    import audit  # scripts/audit.py — stdlib-only, read-only (same import gen_status.py does)
...
def check_statefresh_part1(root):
    """Part 1 — block freshness: `python3 scripts/gen_status.py --check` must exit 0 (every
    generated block is fresh vs capability-matrix.json)."""
    gen_status = os.path.join(root, "scripts", "gen_status.py")
    if not os.path.exists(gen_status):
        return ["scripts/gen_status.py missing"]
...
def check_statefresh(root):
    ...
    if truths is None:
        errs.append("benchmarks/capabilities.json or scripts/audit.py unavailable — "
...
def check_stepcount(root):
    steps = globmod.glob(os.path.join(root, "docs", "workflow", "steps", "[0-9]*.md"))
    n = len(steps)
    if n == 0:
        return [f"no docs/workflow/steps/*.md found under {root}"]
    errs = []
    for rel in STEPCOUNT_DOCS:
        ...
        for m in re.finditer(r"\b(\d+)\s+steps?\b(?!-)", open(p, encoding="utf-8").read()):
            if int(m.group(1)) not in (n,):
                errs.append(f"{rel}: says '{m.group(0)}' but docs/workflow/steps has {n} files")
    return errs


def check_hygiene(root):
    errs = []
    files = _md_files(root, ("docs/workflow", ".claude/skills", ".claude/rules", "README.md"))
    for f in files:
...
def main():
    global REPO
    REPO = str(require_repository_root())
    ...
    ref_files = _md_files(REPO, ("docs/workflow", ".claude/skills")) + [
    ...
    if args.stage:
        ...
        st_files = [f for f in _md_files(st, ("docs/workflow", ".claude/skills"))
                    if os.path.basename(f) != "distribution.md"] + [
