...
(`docs/development/distribution.md`); never reference them as examples in the agent-facing docs.
...
2. Create the skeleton — including the TWO LEDGERS the check-in system requires from birth:
   ...
   ```
   (`docs/workflow/checklists/check-ins.md` makes the DEVIATIONS ledger mandatory and `verify_pack.py`
...
3. Populate `inputs/`:
   ...
   - a **copy** of the param card (never the pristine original) — per
     `docs/workflow/checklists/model-cards.md` (incl. the EWKino recipe for chargino/neutralino);
...
5. Then follow `docs/workflow/README.md` **steps 3→9**: `run-stage` to generate/shower/analyze,
   ...
   **MANDATORY step-9 verification panel** (`verification-panel` skill) before anything is
   delivered. Fill `RESULT.md` (template: `docs/workflow/run-directory.md`).
...
`docs/workflow/checklists/choosing-routine.md`. `[judgment]` for the model/card/statistics choices
