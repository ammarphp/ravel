...
The panel catches OUR mistakes before the physicist sees them (`workflow/steps/09-verify.md` +
`checklists/verification-panel.md` govern).
...
$CONDA run -n rivet python trial-runs/_infrastructure/verify_pack.py <rundir>   # exit 1 = FAIL
```
...
| "matches / consistent with the publication" | its ladder rung row — WHICH published number, WHICH artifact, WHAT tolerance (`workflow/checklists/verification-ladder.md`; catalogue A6) | prose agreement with no artifact behind it |
...
list starts at `framework/FAILURE-CATALOGUE.md` (walk A1→D3 against this run: figure actually
...
`inputs/checkin_deck.json` and run `python3 trial-runs/_infrastructure/validate_checkin.py
