#!/usr/bin/env python3
"""framework/green_board.py -- the ONE aggregate 'make green' board for the workflow-adherence spine.

Runs, from the repo root, the whole green stack in one exit code:
  spine_sim           python3 framework/spine_sim/run_spine_sim.py --require-all   (every G0-G27 fires)
  check_agent_surface python3 trial-runs/_infrastructure/check_agent_surface.py    (routing/docs coherent)
  validate_run_state  python3 trial-runs/_infrastructure/validate_run_state.py --selftest  (the 19 lifecycle/invariant cases)
  audit               python3 framework/audit.py                                   (readiness report, informational)

These are the REAL as-built L6 checks for the workflow-adherence spine. The legacy CR-030..043 board
`framework/verify_fixes.py` is intentionally NOT a rung here: its CR-039 line hard-validates the frozen
...
RUNGS = [
    ("spine_sim", "python3 framework/spine_sim/run_spine_sim.py --require-all", False),
    ("check_agent_surface", "python3 trial-runs/_infrastructure/check_agent_surface.py", False),
    ("validate_run_state", "python3 trial-runs/_infrastructure/validate_run_state.py --selftest", False),
    ("audit", "python3 framework/audit.py", True),
]
SELF_DRIVE_RUNG = ("self_drive", "python3 framework/spine_sim/clean_room.py --live --checkin 2", False)
...
def main(argv=None) -> int:
    ...
    if args.rungs_json:
        ...
        rungs = [SELF_DRIVE_RUNG] + [
            ("spine_sim", "python3 framework/spine_sim/run_spine_sim.py --require-all --with-self-drive",
