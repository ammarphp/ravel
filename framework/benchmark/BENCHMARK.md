...
```bash
python3 framework/benchmark/run_benchmark.py --fast    # fast_case only (~7 s) — per-session smoke gate
python3 framework/benchmark/run_benchmark.py --full    # all cases (~25 s) — the milestone gate
...
tool `framework/audit.py` does NOT share this overwrite-every-run behavior — it is read-only by
...
`trial-runs/2026-*` run dirs by policy (`workflow/DISTRIBUTION.md`), so `cases.json`'s inputs
