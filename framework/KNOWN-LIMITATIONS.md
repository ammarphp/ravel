...
> **`framework/LIMITATIONS-TRIAGE.md`** — `investigated-to` grade (evidence-cited), the
...
- **Higher-order σ is a flat k-factor, not a recomputation.** (Updated Session 2 — the old "LO
  ...
  WG NLO+NLL/NNLL k in `framework/benchmark/cases.json`, the ONLY k authority; per-run RESULT.md
...
- **Shower tune: certified runs use Pythia 8.312's default Monash 2013, not ATLAS's A14.** The
  ...
  emulation (A14/CUETP8M1 by era) was adopted for NEW runs only (`framework/STATUS.md`). Numbers:
  `framework/interrogations/shower-decay.md`.
...
- **R6 visual fidelity is TWO-TIER since 2026-07-07 (CR-016): layout hygiene is now
  ...
  are produced in the shared mplhep house style (`trial-runs/_infrastructure/mplhep_style.py`) and
  eyeball-verified against `workflow/checklists/plot-criteria.md`; the benchmark's provenance gate
  ...
  scoring of the overlay itself (`framework/benchmark/BENCHMARK.md` hooks).
...
- **Counting model is an approximation.** For analyses without a published likelihood, the per-SR
  ...
  --fitted-bkg`, rank 2.5 in `workflow/checklists/data-acquisition.md`) over the REF integral with
...
- **Result-quality fixes land via the CHANGES-REGISTRY** (`framework/CHANGES-REGISTRY.md`):
...
- **Complex routines: demonstrated, not yet broad.** The recursive-jigsaw EWK search
  ...
  SR; `workflow/checklists/complex-analysis.md` documents the per-region / cutflow-only handling.
...
- **Native SimpleAnalysis backend covers a PORTED SET, not all routines (CR-005 generalized
  ...
  `workflow/reference/native-pipeline.md`). Remaining honest gaps: unported routines still take
...
- **Recursive-jigsaw EWK search — done.** The C1N2→WZ (300,100) run on `ATLAS_2018_I1676551`
  ...
  (`workflow/checklists/model-cards.md`).
