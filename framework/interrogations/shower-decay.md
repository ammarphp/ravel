...
| SD-D4 | major (doc accuracy) | `.claude/rules/madgraph-pythia.md` | **MODSEL rule was factually wrong** (S4-validated): MadGraph does NOT write MODSEL into banners, and Pythia 8.312 imports SLHA DECAY tables regardless of MODSEL — the real silent failure is **width-only/empty DECAY tables** (and MSOFT-derived wrong spectra) | **FIXED**: rule rewritten; actionable core kept (always verify first-LHE-event masses + decay products pre-shower, now automated via `lhe_check.py`, referenced). Same stale claim still echoed in `CLAUDE.md` (gotcha line) and `workflow/checklists/model-cards.md` ("ignores the SLHA DECAY tables") — **outside the approved edit list; flagged for the orchestrator** (the actionable advice there, "always include MODSEL", remains correct) |
...
Canonical template (recommendation, not imposition): **`workflow/reference/shower-config-template.cfg`**
...
All but the FSR check are now in `framework/KNOWN-LIMITATIONS.md` (Physics fidelity, two new entries).
...
| Canonical shower-cfg template (SD-D2) | `workflow/reference/shower-config-template.cfg` (new; tune commented + Monash-default documented; hygiene grep clean — no trial-run references) |
...
- **Stale MODSEL echoes outside the approved file set**: `CLAUDE.md` gotcha line ("a missing MODSEL
  silently disables SLHA decays") and `workflow/checklists/model-cards.md` ("ignores the SLHA DECAY
...
`python3 framework/benchmark/run_benchmark.py --full` → **exit 0, 4/4 OK**, metrics unchanged vs the
