...
`workflow/checklists/plot-criteria.md`. Gate state at stage close: `run_benchmark.py --full`
...
- `trial-runs/_infrastructure/mplhep_style.py` — **NEW** shared house-style module (style+Type-42,
  Okabe-Ito + process map, `tick_hygiene`, `smart_legend`, mplhep-absent fallback).
- `trial-runs/_infrastructure/overlay_on_data.py` — hygiene block (ticks/legend/offset/ratio-sync),
  `--sig-scale`, stack validation + CB-safe colours, label wrapping, mplhep-1.2 `text=` fix.
- `trial-runs/_infrastructure/name_plots.py` — input validation (loud stderr + exit 1).
- `trial-runs/_infrastructure/plot_simpleanalysis.py` — house style, readable `barh` SR labels,
  argv-derived lumi label, Okabe-Ito, 200 dpi.
- `framework/benchmark/cases.json` — squark-merged `provenance.require_files` += the new 2jl overlay
  (only edit to the registry).
- 3× `named/INDEX.md` (squark-pair row update; gluino + merged overlay rows appended).
- `framework/KNOWN-LIMITATIONS.md` — R6 entry (above).
- Figures: 3 regenerated in place + 1 new (+ 21 merged-run rivet-mkhtml plots + named copies).

## Final gate state

`python3 framework/benchmark/run_benchmark.py --full` → **exit 0**, GATE OK, 4/4:
