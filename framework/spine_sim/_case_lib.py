#!/usr/bin/env python3
"""framework/spine_sim/_case_lib.py -- the shared toolkit every spine_sim case imports.
...
REPO = os.path.dirname(os.path.dirname(HERE))          # framework/spine_sim -> framework -> repo
INFRA = os.path.join(REPO, "trial-runs", "_infrastructure")
for _p in (INFRA, HERE):
    if _p not in sys.path:
        sys.path.insert(0, _p)

import validate_task_contract  # noqa: E402  (exists today)
import validate_run_state as vrs  # noqa: E402  (exists today)

VRS_PY = os.path.join(INFRA, "validate_run_state.py")
...
def tool_path(name):
    for base in (INFRA, os.path.join(REPO, "framework")):
...
def spike_check(spk):
    art = os.path.join(REPO, "framework", "spine", "spikes", f"{spk}.json")
