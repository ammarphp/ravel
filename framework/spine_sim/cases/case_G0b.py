...
completion channel), so `spike_probe.py --spike SPK-2 --check` re-derives PASS and exits 0. Drive
...
def run():
    ...
    art = os.path.join(L.REPO, "framework", "spine", "spikes", "SPK-2.json")
    for p in (probe, art):
    ...
    L.gate_fired(p.returncode == 0,
                 f"spike_probe --spike SPK-2 --check exit {p.returncode}, expected 0 (recorded PASS); "
