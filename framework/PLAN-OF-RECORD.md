...
- the FAITHFUL grid = **ATLAS's own published lattice** (HEPData ins1767649 Fig 44ab = 12 slepton masses
  ...
  Spec `trial-runs/_infrastructure/specs/slepton_bino_fig3_FULL.json` = the **52 on-grid points with Δm ≥ 2**
...
self-drive) and 5 (native generality) remain, tracked as CR-006/CR-005 in `framework/CHANGES-REGISTRY.md`
(the residual's only lever = the deferred CR-004 rescan). The physicist-facing layer (INITIATE/check-ins/
verification panel) has shipped; **the active workstream is `framework/OPERABILITY-CHARTER.md` (v1.1)**.
...
1. **Doc reconciliation (concern 1).** Make native the SINGLE default at the step-4 SimpleAnalysis
   ...
   timing (MG-only "minutes" → measured full-chain ~30–50 min). Re-grep `workflow/` for
...
detail lives in `framework/CHANGES-REGISTRY.md` CR-020 and CR-005. Recorded here per PLAN-OF-RECORD's
...
**`framework/CAPABILITY-ROADMAP.md`**. CURRENT readiness/capability STATE is owned by
`framework/capability-matrix.json` (via `audit.py`/`AUDIT.md`); this plan and the roadmap carry
