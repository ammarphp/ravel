...
> **"Development session: execute framework/OPERABILITY-CHARTER.md end-to-end."**
...
> `framework/CHANGES-REGISTRY.md` (the dated fix registry, seeded) and
> `framework/FAILURE-CATALOGUE.md` (real incidents → guards; the Tier-B attack list).
...
- **Workflow** — the agent-facing procedure that drives the pipeline: `workflow/WORKFLOW.md` →
...
- **Harness** — the *executable* control layer: scripts/validators/schemas that force correctness
  instead of trusting prose (`trial-runs/_infrastructure/*.py`, `framework/benchmark/`, `audit.py`).
...
- **Scaffolding** — everything above plus routing surfaces (`CLAUDE.md`, `AGENTS.md`,
  `workflow/INITIATE.md`): the structure that lets ANY session land in the right procedure.
...
- **Routing**: `CLAUDE.md` (two-session fork at top), `AGENTS.md` (Codex mirror), `workflow/INITIATE.md`
  (physicist entrypoint), check-in system `workflow/checklists/check-ins.md` (plan check-in w/ figure
...
- **Verification**: step `09-verify.md` + `checklists/verification-panel.md` (mechanical tier +
  ...
  `framework/benchmark/run_benchmark.py --fast|--full` (locked gate); `framework/audit.py`.
...
- **Evidence** (dev-only): two docx reviews (`HEP Workflow Codex Audit`, `Architectural
  ...
  `framework/PLAN-OF-RECORD.md`, `framework/KNOWN-LIMITATIONS.md`, `framework/STATUS.md`.
...
**`framework/FAILURE-CATALOGUE.md`**; the step-9 Tier-B verification adversary uses it as an attack
...
2. **Product contract**: NEW `PRODUCT-CONTRACT.md` (recommended by the Codex-audit docx; still
...
3. **Skills** (the judgment, operationalized): mirrored `.claude/skills/` ↔ `.agents/skills/`
   ...
   - **EXISTING-SKILLS AUDIT (concrete staleness, verified 2026-07-06 — fix these, don't just
     ...
     · `new-analysis` — says "follow workflow/WORKFLOW.md (steps 3→7)", omitting step 8 (scan) and
     ...
     · `run-stage` — covers Rivet-only analyze: no SimpleAnalysis/Delphes/NATIVE-backend path
       ...
       instead of running the mandated `trial-runs/_infrastructure/lhe_check.py` guard; cites a
...
4. **Harness** (weak models are FORCED through scripts, not trusted with prose): keep tools where
   ...
   - `check_agent_surface.py` — CLAUDE↔AGENTS agreement; every referenced skill/file exists; skill
     ...
     no trial-run leakage in workflow/.
...
5. **Records/evidence**: unchanged policy (trial-runs dev-only, quarantine never delete), plus the
   **dated fix-registry** (`framework/CHANGES-REGISTRY.md`, ADR-style: ID, date, what, why, where
...
|---|---|---|
| generation → shower | `lhe_check.py` (masses/MODSEL/BR rows/weights/merged-flag) | `trial-runs/_infrastructure/lhe_check.py`, mandatory pre-shower |
...
| artifacts (step 9, tier A) | `verify_pack.py` (numbers trace, figures exist, coverage, DEVIATIONS present) | `trial-runs/_infrastructure/verify_pack.py` |
| regression | `run_benchmark.py --fast` (per session) / `--full` (any physics change) | `framework/benchmark/` |
...
- **After compaction or a restart, re-anchor from files, never from an auto-summary**: re-read
  `workflow/INITIATE.md` (physicist) or the dev read order → the run's `task_contract.json` →
...
it must land in repo files (workflow/, framework/, skills, or the registry). The gap hunts found
...
`reference/native-pipeline.md`, a dev-only ops note, PRODUCT-CONTRACT.md respectively).
...
- **P0 freeze**: git status clean-point; run `audit.py --check` (read-only by default — no
  ...
  editing.
- **P1 audit** (deliverable `framework/AUDIT-OPERABILITY.md`, severity-ranked): the §3 failures +
  ...
  **Concrete P1 targets (from the 2026-07-06 gap hunts; verify each is still live, then fix):**
  - `framework/PLAN-OF-RECORD.md` **currency** — a dated supersession STATUS block landed
  ...
  - `framework/benchmark/cases.json` inputs point at dev run dirs absent from the clone —
  ...
  - `workflow/DISTRIBUTION.md` policy vs `export_distribution.sh` drift: `shared/` listed
  ...
  - Stale instrument runs + state docs: `framework/AUDIT.md` (generated pre-09-verify, "8 step
    files"); `framework/STATUS.md` session-log entries missing for the last four landmark commits,
    pre-charter open items, the unresolved +3j parting job; residual step-count "8 steps" spots
    (STATUS, `shared/agent/ORCHESTRATION.md`, AUDIT.md R8) — README fixed v1.1; make step-count
...
- **P2 build**: the skills set + mirror sync, the harness scripts, PRODUCT-CONTRACT.md,
...
- **P4 prove — ROUTING EVALS** (`framework/ROUTING-EVALS.md`): the seven REAL physicist prompts
