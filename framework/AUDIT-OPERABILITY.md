...
> Deliverable of `framework/OPERABILITY-CHARTER.md` §5-P1, produced 2026-07-06 by six parallel
...
- **A1-04** CLAUDE.md vs AGENTS.md dev read-list disagreement (ORCHESTRATION-first vs
  ...
  **DIRECTORY.md → framework/STATUS.md → framework/PLAN-OF-RECORD.md** (+ optional dev-only
...
- **A1-05 = A3-03** Readiness numbers disagree on disk NOW: regenerated AUDIT.md 97% vs 100%
  ...
  shared/agent/ORCHESTRATION.md. → Fix the audit instrument (A3-02), regenerate, update all,
...
- **A1-07 = A5-08** Export hygiene grep scans only staged workflow/ — the dated dev-style
...
- **A1-09** shared/agent/ORCHESTRATION.md: "8 steps" ×2 + describes SA path as containerized.
...
- **A1-14** DIRECTORY.md missing rows for 4 on-disk run dirs + workflow/reference/native-pipeline.md.
- **A2-07** No checkpoint/RESUME rule anywhere in workflow/ (§4c gap) despite hours-scale scans.
...
- **A6-07** §4d-d homeless: the archetype census (4 archetypes / 94% coverage, 2026-06-15) —
  the number that settled the design — recorded nowhere. → PRODUCT-CONTRACT.md + one-line
...
- Dead-path scan of workflow/ clean: all 35 referenced infra scripts/specs/templates exist
...
2. **P2d code fixes**: CR-001 (pyhf_exclude downward bracket + `at_mu_floor` + harvest guard +
   ...
   both drivers (A6-03).
3. **PRODUCT-CONTRACT.md** (charter §4.2 + archetype census + stat/fidelity taxonomy).
...
5. **P2c skills**: seven new (physicist-intake, route-analysis, run-scan, figure-contract,
   ...
   .agents mirror.
6. **Model-tier policy**: [Opus] → [judgment]+behavior across workflow/ (A2-02 classification).
7. **Memory landings**: framework/interrogations/generality.md · 0x7FFFFFFF rationale (docstring
