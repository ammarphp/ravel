...
> `framework/CHANGES-REGISTRY.md` (CR-001/CR-002; dev repo: OPERABILITY-CHARTER §3 F6). New
...
(`framework/CHANGES-REGISTRY.md` CR-004 FINAL).
...
(`framework/PLAN-OF-RECORD.md` drift list). Guard: step 8's deliverable is the 2-D contour
...
over workflow/. Guard: native is THE default (`reference/native-pipeline.md`, steps 04/08); every
...
physicist trials. Guard: the two-session fork at the top of CLAUDE.md/AGENTS.md + `workflow/INITIATE.md`
...
> D-series above; the map is authoritative in `framework/spine_sim/README.md` (G0a–G27).
...
back to the agent; the step-end self-drive rule lives in `workflow/steps/03-generate.md` ("Self-drive at
...
uncaught. Caught: a staged export tree whose `workflow/*.md` references a nonexistent path. Guard: **G20** —
...
`framework/spine_sim/clean_room.py --live` (CR-106) launches the un-hinted self-drive and records
...
When running the step-9 verification panel's **Tier B** (`workflow/checklists/verification-panel.md`),
...
- **N10 — HEPData bulk endpoints blocked server-side from this host; harvest rerouted through the in-app browser's AJAX endpoint.**
  ...
  workflow/checklists/data-acquisition.md now carries the browser-route recipe: record JSON via
...
- **A9 — a physicist-facing spot-check quoted a survey read-off where the exact harvested value existed.**
  ...
  workflow/checklists/summary-plot.md now requires identity/spot-check rows to use exact harvested
