...
Import the module under test BY FILE PATH (the repo-root py.py shadows the `py` package); run from /tmp:
    REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_validate_run_state_lifecycle.py" -q
"""
...
def test_verify_provenance_rejects_fingerprint_mismatch():
    vrs = _load()
    import importlib.util
    sp_path = REPO / "trial-runs" / "_infrastructure" / "sr_plausibility.py"
