"""framework/build_evidence.py + framework/check_evidence.py -- PRODUCT-CONTRACT section 7
...
or the export aborts. This pins:
  ...
  - the live dev-tree integration: `python3 framework/check_evidence.py --check` exits 0 against
    the manifest this task ships (evidence_manifest.json at the repo root).
...
BUILD_EVIDENCE_PY = REPO / "framework" / "build_evidence.py"
CHECK_EVIDENCE_PY = REPO / "framework" / "check_evidence.py"
...
def test_is_shipped_framework_row_by_row():
    be = _load_build_evidence()
    assert be.is_shipped("framework/STATUS.md") is True
    assert be.is_shipped("framework/DECISION-SHAPE-FIT.md") is True
    assert be.is_shipped("framework/benchmark/cases.json") is True
    assert be.is_shipped("framework/interrogations/generality.md") is True
...
def test_is_shipped_top_level():
    be = _load_build_evidence()
    assert be.is_shipped("workflow/reference/native-pipeline.md") is True
    assert be.is_shipped("PRODUCT-CONTRACT.md") is True
...
def test_materialize_claim_hashes_existing_candidates(tmp_path):
    be = _load_build_evidence()
    (tmp_path / "trial-runs" / "_infrastructure").mkdir(parents=True)
    art = tmp_path / "trial-runs" / "_infrastructure" / "tool.py"
    art.write_text("print('hi')\n")
    spec = {"claim_id": "C1", "source": "test", "headline": "h", "status": "served",
            "gate": "g", "candidates": [("trial-runs/_infrastructure/tool.py", "engine-code")],
            "surrogate": None}
...
def test_materialize_claim_falls_back_to_surrogate_when_all_candidates_missing(tmp_path):
    ...
    surrogate_file = tmp_path / "framework" / "capability-matrix.json"
    ...
    assert len(claim["artifacts"]) == 1
    assert claim["artifacts"][0]["path"] == "framework/capability-matrix.json"
...
def test_materialize_claim_skips_surrogate_when_a_real_artifact_already_ships(tmp_path):
    be = _load_build_evidence()
    (tmp_path / "trial-runs" / "_infrastructure").mkdir(parents=True)
    (tmp_path / "trial-runs" / "_infrastructure" / "tool.py").write_text("x")
    spec = {"claim_id": "C2", "source": "test", "headline": "h", "status": "served", "gate": "g",
            "candidates": [("trial-runs/_infrastructure/tool.py", "engine-code")],
            "surrogate": ("framework/capability-matrix.json", "matrix-entry")}  # deliberately absent
    claim = be.materialize_claim(spec, root=tmp_path, warn=lambda _m: None)
...
def test_enumerate_specs_matches_live_repo_sources():
    be = _load_build_evidence()
    matrix = json.loads((REPO / "framework" / "capability-matrix.json").read_text())
    cases_doc = json.loads((REPO / "framework" / "benchmark" / "cases.json").read_text())
...
def test_check_claim_partial_with_shipped_artifact_passes(tmp_path):
    ce = _load_check_evidence()
    _write(tmp_path, "framework/STATUS.md", "status")
    claim = {"claim_id": "P2", "status": "partial", "artifacts": [
        _artifact("trial-runs/x/RESULT.md", "x", shipped=False),
        _artifact("framework/STATUS.md", "status", shipped=True),
    ]}
    verdict, detail = ce.check_claim(claim, tmp_path)
    assert verdict == "PASS", detail
...
def test_cmd_check_fails_loud_when_manifest_missing(tmp_path, capsys):
    ce = _load_check_evidence()
    ce.ROOT = str(tmp_path)   # no evidence_manifest.json under here
...
def test_live_evidence_manifest_checks_clean():
    ...
    assert manifest_path.is_file(), \
        "evidence_manifest.json missing at repo root -- run `python3 framework/build_evidence.py --write`"
