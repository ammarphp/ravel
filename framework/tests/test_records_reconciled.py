...
def _load_check_agent_surface():
    path = REPO / "trial-runs" / "_infrastructure" / "check_agent_surface.py"
...
def test_agent_surface_green():
    ...
    r = subprocess.run(
        [sys.executable, "trial-runs/_infrastructure/check_agent_surface.py"],
...
def test_dirmap_flags_dangling_row_the_g20_trigger(tmp_path):
    ...
    # and the DISTRIBUTION-tree behavior: same dangling row, no sentinel -> WARN, not error
    (tmp_path / "ORCHESTRATION.md").unlink()
    errs2, warns2 = cas.check_dirmap(str(tmp_path))
    assert not errs2 and any("dev-only" in w for w in warns2), (
        f"dist-tree dirmap should WARN, got errs={errs2} warns={warns2}")
