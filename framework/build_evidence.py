#!/usr/bin/env python3
"""Build evidence_manifest.json + EVIDENCE.md -- PRODUCT-CONTRACT section 7 (CR-030): every
SHIPPED headline/served claim must map to a shipped, sha256-checksummed artifact on disk, or the
export aborts (framework/check_evidence.py's job, Task 6.2). This script is the BUILDER half: it
...
Claim sources:
  1. `framework/capability-matrix.json` `prompts` entries carrying an `evidence_artifacts` list
  ...
  2. `framework/benchmark/cases.json` `cases[].provenance.require_files` -- reused verbatim
  ...
  3. HEADLINE_CLAIMS below -- a small curated block for engine-layer claims that live in SHIPPED
     docs (`workflow/reference/native-pipeline.md`, `framework/STATUS.md`, ...) but are not
...
`shipped` classification mirrors `trial-runs/_infrastructure/export_distribution.sh`'s ACTUAL copy
list (the ground truth for what leaves the machine at export time) -- NOT the prose table in
`workflow/DISTRIBUTION.md`, which has visibly drifted from that script (e.g. the script ships
...
Usage:
    python3 framework/build_evidence.py [--write] [--timestamp T] [--commit SHA]

--write is the default action (this generator has no separate read-only report mode -- writing
IS the point; `framework/check_evidence.py --check` is the read-only verifier). `--timestamp`/
...
# Mirrors trial-runs/_infrastructure/export_distribution.sh's `for f in ...:cp` / `for d in
# ...:rsync` lists exactly. Re-derive this set if that script's file lists change (a drift check
# is out of this task's scope; the script itself remains the single ground truth it always was).
_SHIPPED_FRAMEWORK_FILES = {
    "PLAN-OF-RECORD.md", "STATUS.md", "AUDIT.md", "audit.py", "KNOWN-LIMITATIONS.md",
    "ENVIRONMENT.md", "CHANGES-REGISTRY.md", "FAILURE-CATALOGUE.md", "AUDIT-OPERABILITY.md",
    "ROUTING-EVALS.md", "CAPABILITY-ROADMAP.md", "capability-matrix.json",
    "LIMITATIONS-TRIAGE.md", "DECISION-SHAPE-FIT.md", "OPERABILITY-CHARTER.md",
}
_SHIPPED_FRAMEWORK_DIRS = ("benchmark", "validation", "crosscheck", "interrogations")
_SHIPPED_TOP_FILES = {"README.md", "CITATION.cff", "LICENSE", "DIRECTORY.md", "AGENTS.md",
...
def is_shipped(relpath):
    ...
    relative path. False means dev-only *today* -- a future task may still carve it into a
    curated evidence subset; that is a policy decision for the caller (via a surrogate), not for
    ...
    if parts[0] == "framework":
        if len(parts) == 2 and parts[1] in _SHIPPED_FRAMEWORK_FILES:
            return True
        return len(parts) > 2 and parts[1] in _SHIPPED_FRAMEWORK_DIRS
    if parts[0] in _SHIPPED_TOP_FILES:
        return True
    return parts[0] in _SHIPPED_TOP_DIRS


# --------------------------------------------------------------------------- #
# per-artifact role inference (cosmetic -- EVIDENCE.md readability only, never gates a verdict)
# --------------------------------------------------------------------------- #
...
HEADLINE_CLAIMS = [
    {
        ...
        "doc_source": "workflow/reference/native-pipeline.md",
        "artifacts": ["trial-runs/2026-06-16_slepton_200-150_native/output/EwkCompressed2018.txt"],
        "surrogate": "workflow/reference/native-pipeline.md",
    },
    {
    ...
    },
    {
        ...
        "doc_source": "framework/STATUS.md",
        "artifacts": ["trial-runs/sleptonscan_fig3_SCAN/scan.json"],
        "surrogate": "framework/STATUS.md",
    },
]
    ...
    for key, v in sorted(matrix.get("prompts", {}).items()):
        ...
        if not artifacts:
            raise BuildError(
                ...
                f"list (add one to framework/capability-matrix.json, see this script's module "
                f"docstring source 1)")
        specs.append({
            "claim_id": key,
            "source": f"framework/capability-matrix.json:prompts.{key}",
            "headline": PROMPT_HEADLINES.get(key, key),
            "status": v.get("status"),
            "gate": _matrix_gate_label(v.get("gate")),
            "candidates": [(a, _infer_role(a)) for a in artifacts],
            "surrogate": ("framework/capability-matrix.json", "matrix-entry"),
        })
    return specs
...
def headline_specs():
    ...
    for hc in HEADLINE_CLAIMS:
        specs.append({
            ...
            "source": f"framework/build_evidence.py:HEADLINE_CLAIMS.{hc['claim_id']} "
...
def build_manifest(root=ROOT, timestamp="", commit=""):
    with open(os.path.join(root, "framework", "capability-matrix.json"), encoding="utf-8") as f:
        matrix = json.load(f)
    with open(os.path.join(root, "framework", "benchmark", "cases.json"), encoding="utf-8") as f:
...
def render_evidence_md(manifest):
    lines = [
        "# EVIDENCE -- served-claim to shipped-artifact map", "",
        "_Generated by `framework/build_evidence.py` from `capability-matrix.json` prompts, "
        "`benchmark/cases.json`, and a curated `HEADLINE_CLAIMS` block. "
    ...
    lines += ["", f"**{len(manifest['claims'])} claim(s)** ({n_served} served/served-with-"
              ...
              f"`framework/check_evidence.py --check`)."]
    return "\n".join(lines) + "\n"
