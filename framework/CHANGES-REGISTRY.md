...
> Binding per `framework/OPERABILITY-CHARTER.md` §7: **every change lands here** — pending fixes are
...
> Real incidents behind these entries: `framework/FAILURE-CATALOGUE.md`.
...
- **What:** PRODUCT-CONTRACT §7 gains a claim-integrity rule: every headline/served claim in the
  SHIPPED docs (README, `workflow/reference/*`, PRODUCT-CONTRACT) must map to a shipped,
  sha256-checksummed artifact via `evidence_manifest.json`; `export_distribution.sh` runs
...
- **Where-embedded:** PRODUCT-CONTRACT §7 (this entry). `evidence_manifest.json` +
...
- **Verify:** `python3 framework/check_evidence.py --check` (read-only; checksums include
  `framework/benchmark/cases.json` as a shipped BENCH_* artifact, so this can show a TRANSIENT
...
- **Status:** REGISTERED (open) — scope lands now; build is a separate later task.
- **Verify:** `python3 trial-runs/_infrastructure/summary_audit.py --selftest && python3 trial-runs/_infrastructure/summary_audit.py --rundir trial-runs/2026-07-06_SURVEY_hvt-zprime-ww-lowmass`
...
- **Verify:** `python3 trial-runs/_infrastructure/validate_run_state.py --selftest`
...
- **Verify:** `python3 trial-runs/_infrastructure/project_limits.py --selftest && python3 trial-runs/_infrastructure/replane.py --selftest`
...
- **Where-embedded:** PRODUCT-CONTRACT §2 + §5 (this entry); CR-020 (the D1 route this labels) and
  the D2 LLP design (`workflow/reference/effmap-folding.md`).
...
- **Verify:** `python3 trial-runs/_infrastructure/validate_task_contract.py --selftest`
...
- **What:** PRODUCT-CONTRACT §5 gains a `capability-status-legitimacy` row: a `served`/
  ...
  `framework/audit.py` reconcile over the per-prompt gate.
...
- **Status:** REGISTERED (open) — scope lands now; build is a separate later task.
- **Verify:** `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_capability_gate.py" -q`
...
- **What:** PRODUCT-CONTRACT §8 gains two additions (appended, the pre-existing binding-chain
  ...
  `framework/capability-matrix.json` is the single source of STATE (served/partial/unbuilt;
...
- **Why:** three documents (matrix/PLAN-OF-RECORD/ROADMAP) can each look authoritative; without a
  ...
  `workflow/checklists/verification-panel.md` — §8 now names the full chain in one place.
...
- **Status:** REGISTERED (open) — scope lands now; `validate_run_state.py` build is a separate
  later task.
- **Verify:** `python3 trial-runs/_infrastructure/check_agent_surface.py && python3 trial-runs/_infrastructure/validate_run_state.py --selftest`
...
- **What:** `framework/audit.py` gains argparse — default `--check` computes + prints readiness and
  writes NOTHING; `--write [--out]` regenerates `AUDIT.md`. `framework/benchmark/run_benchmark.py`
...
- **Where-embedded:** `framework/audit.py`, `framework/benchmark/run_benchmark.py`,
  `framework/tests/test_audit_readonly.py`, `framework/tests/test_run_benchmark_out.py`; call-site
...
- **Verify:** `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_audit_readonly.py" "$REPO/framework/tests/test_run_benchmark_out.py" -q` (read-only: subprocess-runs `audit.py` with no args only; `run_benchmark.py` is exercised via argparse introspection only, no physics execution, so this never touches `framework/benchmark/`'s shared state)
...
- **What:** new `framework/gen_status.py` machine-owns the readiness/capability PROSE inside
...
- **Where-embedded:** `framework/gen_status.py` + the 13 reconciled docs; the enforcement guard
  is `trial-runs/_infrastructure/check_agent_surface.py`'s new `statefresh` check (Phase 2B):
  ...
  sentences that assert the opposite). Tests: `framework/tests/test_statefresh.py` (clean passes,
  stale claim caught, history line not flagged).
- **Status:** EMBEDDED (2026-07-08) — generator + reconciliation + enforced statefresh guard.
- **Verify:** `python3 framework/gen_status.py --check && python3 trial-runs/_infrastructure/check_agent_surface.py`
...
- **What:** the full lifecycle-gate build CR-032/CR-036 registered as scope-only, landed in three
  ...
  **R5-before-limit-ships**, **likelihood-selection-pairing** via `pairing_check.py`, blocked-
  ...
  what a run is missing. (3) **Wiring + flagship backfill** (3.3): `workflow/steps/09-verify.md`
  Tier A now runs `validate_run_state.py --rundir` BEFORE `verify_pack.py`;
  `workflow/checklists/verification-panel.md` gains the corresponding checkbox;
  `workflow/steps/07-exclude.md` Mode A notes the `pairing_check.py` pre-limit gate for
...
- **Where-embedded:** `trial-runs/_infrastructure/{shape_fit.py,pairing_check.py,
  validate_run_state.py}`; tests `framework/tests/{test_shape_fit_json.py,
  test_pairing_check_json.py,test_validate_run_state.py}` (+ `validate_run_state.py --selftest`,
  7 embedded fixture cases); `workflow/steps/09-verify.md`, `workflow/checklists/
  verification-panel.md`, `workflow/steps/07-exclude.md`; `trial-runs/sleptonscan_fig3_SCAN/
...
- **Verify:** `python3 trial-runs/_infrastructure/validate_run_state.py --selftest && python3 trial-runs/_infrastructure/validate_run_state.py --rundir trial-runs/sleptonscan_fig3_SCAN` (the second call is the flagship dogfood claim itself: exit 0, verdict=WARN — only the pre-cleared likelihood-pairing WARN)
...
- **What:** new `trial-runs/_infrastructure/summary_audit.py` — the summary_plot physics-completeness
...
- **Where-embedded:** `trial-runs/_infrastructure/{summary_audit.py,summary_overlay.py,validate_run_state.py}`,
  `framework/tests/{test_summary_audit.py,test_validate_run_state.py}`, the HVT run dir (survey.json,
...
- **Where-embedded:** `framework/capability-matrix.json` (gate{} per prompt), `framework/audit.py`,
  `framework/tests/test_capability_gate.py` (14 anti-gaming tests). Readiness unchanged (96%, R9 0.64)
...
- **What:** `framework/build_evidence.py` enumerates every served/headline claim (capability-matrix
  prompts' `evidence_artifacts`, benchmark cases' `require_files`, and a curated engine-layer
  HEADLINE_CLAIMS block) → `evidence_manifest.json` + `EVIDENCE.md`, sha256 each (13 claims).
  `framework/check_evidence.py --check [--root <dir>]` fails unless every served claim has a
...
- **Where-embedded:** `framework/{build_evidence.py,check_evidence.py}`, `evidence_manifest.json`,
  `EVIDENCE.md`, `framework/capability-matrix.json` (evidence_artifacts), `trial-runs/_infrastructure/
  export_distribution.sh`, `workflow/DISTRIBUTION.md`, `README.md`, `workflow/reference/native-pipeline.md`,
  `framework/tests/test_check_evidence.py` (22 tests). Verified: `check_evidence --check --root $STAGE`
...
- **What (reconcile):** `framework/OPTION-C-DESIGN.md`'s step-list was written 2026-07-06 and
  ...
  (step-list item 3) is in `workflow/steps/03-generate.md`'s "Beyond signal samples" section
  (~lines 60-67); (2) the `sensitivity-expected-only` stat_mode (step-list item 5's
  "`result_pack.py --stat-mode sensitivity`") is in `trial-runs/_infrastructure/result_pack.py`'s
  `STAT_MODES` tuple (line 72; landed via CR-009); (3) the first-class Option-C step (step-list
  item 2) is `workflow/steps/04-analyze.md`'s "## Option C — NO routine exists" section, backed by
  `workflow/checklists/option-c-judgment.md` (the physics-judgment protocol; supersedes the doc's
...
- **What:** `trial-runs/_infrastructure/pyhf_exclude.py` brackets µ upward from 1 only and never
...
- **Where-embedded (fixed 2026-07-06):** `prepare_native_slepton.py` `read_run_options()` +
  ...
  Doc: `workflow/reference/native-pipeline.md` states the block is applied + the pre-fix caveat.
...
- **Status:** OPEN / DEFERRED (no build scheduled yet; the honest caveat is disclosed in the
  ...
  misrepresented).
- **Verify (once built):** `python3 framework/benchmark/run_benchmark.py --case
...
- **Date:** 2026-07-06. **What:** no enum fit an EXPECTED-only sensitivity study (S/√B,
  ...
  `none-survey` to `result_pack.py` STAT_MODES; PRODUCT-CONTRACT.md carries the taxonomy.
- **Status:** EMBEDDED (2026-07-06).
...
- **What:** every `[Opus]` tag across `workflow/` + `CLAUDE.md` renamed `[judgment]` (63
  sites); the binding three-behavior policy defined ONCE in `workflow/WORKFLOW.md` §Roles
...
- **What:** `scan_contour.py` carried four raw `ax.legend(loc=…)` calls (line/grid/fig3/reldiff
  ...
  target for the `plot_lint.py` machine gate (`framework/CAPABILITY-ROADMAP.md` §7 / W1).
...
- **What:** `audit.py` gains `c_capability()` (R9): reads `framework/capability-matrix.json`
...
- **What:** (a) `reinterpret_db.py --data-select efficiencyMap|upperLimit|all` — restricting the
  ...
  (a minimal ini crashes loadDatabase on the missing `database.path`). (b) `workflow/reference/
...
- **DEFER TRIGGER (recorded 2026-07-08, Task 8.1 — I6):** the D2 per-object efficiency-map
  ...
  Confirmed: `framework/capability-matrix.json`'s `P7_displaced_run3_replane` gate is already
...
- **What:** `workflow/checklists/summary-plot.md` — the no-generation track behind
...
- **Date registered:** 2026-07-07 (checkpoint C12). Spec: `workflow/reference/
...
- **Where-embedded:** `workflow/reference/projection-replane.md` §Projection (likelihood-mode
...
- **Verify:** `python3 trial-runs/_infrastructure/project_limits.py --selftest`
...
- **2-D UL-SURFACE mode added (2026-07-11):** compressed-spectrum searches limit σ vs BOTH a
  ...
  honesty). Embedded: `workflow/reference/projection-replane.md` §Replane 2-D paragraph.
...
- **Date:** 2026-07-07 (continuation-3). `trial-runs/_infrastructure/scan_babysitter.py`.
...
- **What:** generalize `native_simpleanalysis.py` + `prepare_native_slepton.py` +
  ...
  contract in `workflow/reference/native-pipeline.md`.
...
- **ADVANCED (2026-07-07 continuation-3, Opus) — the common cut-based case now generalizes:**
  `trial-runs/_infrastructure/native_sa_generic.py` is a DECLARATIVE native SimpleAnalysis engine
...
- **Status update 2026-08-16: EMBEDDED — the trigger fired (supervisor: pre-publication
  ...
  (d) Recipe + traps embedded: `workflow/reference/native-pipeline.md` §porting; oracle is
...
- **What:** a git worktree `wt-spine-hardening` on branch `spine-hardening` off `harness-phase0-3-rework`,
  ...
  `framework/spine/WORKTREE.md` documenting the recipe.
...
- **Date registered:** 2026-07-09
- **What:** `framework/spine/spike_probe.py` — a stdlib-only, read-only, fail-loud recorder/verifier for the
...
- **Where-embedded:** `framework/spine/WORKTREE.md` (spike-verifier pointer); `DIRECTORY.md` `spine/` row
  (interface + `tests/test_spike_probe.py`). Tests: `framework/tests/test_spike_probe.py` (6) + the built-in
  `--selftest` (14 cases).
- **Status:** EMBEDDED.
- **Verify:** `python3 framework/spine/spike_probe.py --selftest && REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_spike_probe.py" -q`
...
- **What:** Wired three trivial PROBE hooks into the worktree `.claude/settings.json` — `UserPromptSubmit`,
  ...
  card-guard (kept byte-for-byte), and recorded the G0a spike artifact `framework/spine/spikes/SPK-1.json`
...
- **Where-embedded:** `framework/spine/WORKTREE.md` ("Spike outcomes" section — the SPK-1 `unproven`
...
- **Status:** EMBEDDED.
- **Verify:** `python3 framework/spine/spike_probe.py --spike SPK-1 --check framework/spine/spikes/SPK-1.json --json | python3 -c "import json,sys; d=json.load(sys.stdin); assert d['consistent'] and d['verdict']=='unproven', d; print('OK: consistent honestly-recorded unproven G0a')"`
...
- **Date registered:** 2026-07-09
- **What:** Recorded the G0b spike artifact `framework/spine/spikes/SPK-2.json` (`verdict=PASS`,
  ...
  `framework/spine/spikes/SPK-2.evidence.json`). The verifier was first proven armed (red): the same
...
- **Where-embedded:** `framework/spine/WORKTREE.md` ("Spike outcomes" — the SPK-2 PASS verdict, the
...
- **Status:** EMBEDDED.
- **Verify:** `python3 framework/spine/spike_probe.py --spike SPK-2 --check framework/spine/spikes/SPK-2.json`
...
- **Date registered:** 2026-07-09
- **What:** Recorded the G0c spike artifact `framework/spine/spikes/SPK-3.json` (`verdict=PASS`,
  ...
  tolerance that absorbs re-invoke jitter (evidence verbatim in `framework/spine/spikes/SPK-3.evidence.json`).
...
- **Where-embedded:** `framework/spine/WORKTREE.md` ("Spike outcomes" — the SPK-3 PASS verdict, the
...
- **Status:** EMBEDDED.
- **Verify:** `python3 framework/spine/spike_probe.py --spike SPK-3 --check framework/spine/spikes/SPK-3.json`
  (exit 0 iff the recorded verdict is PASS and the artifact recomputes from its embedded evidence).

### CR-050 — HOOK-PRIMACY.json per-branch enforcement decision (L0) (Phase 0 Task 0.6)
- **Date registered:** 2026-07-09
- **What:** Synthesized `framework/spine/HOOK-PRIMACY.json` — the authoritative per-branch
...
- **Where-embedded:** `framework/spine/WORKTREE.md` ("Hook primacy (L0 decision)" — the honest
  per-branch table, the binding rule, and the promotion order); `DIRECTORY.md`
  (`framework/spine/HOOK-PRIMACY.json` row).
- **Status:** EMBEDDED.
- **Verify:** `python3 framework/spine/spike_probe.py --check-primacy framework/spine/HOOK-PRIMACY.json`
...
- **What:** Added `trial-runs/_infrastructure/provenance.py` — the stdlib-only SINGLE source of the
  ...
  `framework/tests/test_provenance.py` (3 cases, TDD-first).
...
- **Status:** EMBEDDED.
- **Verify:** `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_provenance.py" -q`
  (3 passed) and `python3 trial-runs/_infrastructure/provenance.py --selftest` (exit 0).

### CR-052 — workflow_state.py init + run_state.json schema (L1/L2 keystone) (Phase 1 Task 1.2)
- **Date registered:** 2026-07-10
- **What:** Added `trial-runs/_infrastructure/workflow_state.py` — the LIVE per-run state machine (the
  ...
  not-a-dir · 3 no/invalid contract). Test: `framework/tests/test_workflow_state.py` (5 cases,
...
- **Where-embedded:** `DIRECTORY.md` (`_infrastructure/` block, the `workflow_state.py` row after
  `provenance.py`); `workflow/WORKFLOW.md` (Reusable-helpers table — the per-run ledger every gate
  reads).
- **Status:** EMBEDDED.
- **Verify:** `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_workflow_state.py" -q`
  (5 passed) and `python3 trial-runs/_infrastructure/workflow_state.py --selftest` (exit 0).

### CR-053 — workflow_state record + active-rundir resolution (G2 substrate) (Phase 1 Task 1.3)
- **Date registered:** 2026-07-10
- **What:** Added the observer's write path to `trial-runs/_infrastructure/workflow_state.py`: the
  ...
  `framework/tests/test_workflow_state.py` (+6 cases, TDD-first) and 3 new `--selftest` checks (4/5/5a/5b).
...
- **Status:** EMBEDDED.
- **Verify:** `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_workflow_state.py" -q`
  (11 passed) and `python3 trial-runs/_infrastructure/workflow_state.py --selftest` (exit 0).

### CR-054 — workflow_state advance — live precondition gate (G3) (Phase 1 Task 1.4)
- **Date registered:** 2026-07-10
- **What:** Added the lifecycle-cursor DRIVE path to `trial-runs/_infrastructure/workflow_state.py`: the
  ...
  not-a-dir · 3 no run_state.json / no|invalid contract. Test: `framework/tests/test_workflow_state.py`
...
- **Where-embedded:** `DIRECTORY.md` (`_infrastructure/` block, the `workflow_state.py` row — the `advance`
  ...
  `workflow/WORKFLOW.md` (the `workflow_state.py` helper row — a step transition runs `advance --to <stage>`,
  the fallback DRIVE precondition gate); consumed by the Phase-2 DRIVE branch + Task 1.5 `status`/`next`.
- **Status:** EMBEDDED.
- **Verify:** `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_workflow_state.py" -q`
  (14 passed) and `python3 trial-runs/_infrastructure/workflow_state.py --selftest` (exit 0).

### CR-055 — workflow_state status/next/require — read-only fallback gate surface (Phase 1 Task 1.5)
- **Date registered:** 2026-07-10
- **What:** Added the three READ-ONLY query subcommands to `trial-runs/_infrastructure/workflow_state.py`:
  ...
  `framework/tests/test_workflow_state.py` (+4 cases, TDD-first — skill gate FAIL→PASS-after-record,
...
- **Where-embedded:** `DIRECTORY.md` (`_infrastructure/` block, the `workflow_state.py` row — the read-only
  ...
  `workflow/checklists/check-ins.md` (Composing rules — the step-boundary self-check line: at a step
...
- **Status:** EMBEDDED.
- **Verify:** `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_workflow_state.py" -q`
  (18 passed) and `python3 trial-runs/_infrastructure/workflow_state.py --selftest` (exit 0).
...
- **What:** Added the L1 PostToolUse OBSERVER `.claude/hooks/posttooluse-observer.sh` and wired it into
  ...
  UserPromptSubmit/PostToolUse/Stop probe blocks. Fallback twin: `workflow/checklists/check-ins.md`
  ...
  `framework/tests/test_posttooluse_observer.py` (3 cases, TDD-first — records skill; records edit +
...
- **Where-embedded:** `DIRECTORY.md` (`.claude/hooks/` block — a new row for `posttooluse-observer.sh`
  ...
  `workflow/checklists/check-ins.md` (Composing rules — the "Observer fallback" note: the hand-run
  `record` twin + the D-2 compute exception). Consumed by the Phase-2 skill-coverage (G2) Stop branch.
- **Status:** EMBEDDED.
- **Verify:** `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_posttooluse_observer.py" -q`
  (3 passed).
...
- **What:** Added the `--verify-provenance` mode to `validate_run_state.py`: it imports `provenance.py`
  ...
  `framework/tests/test_verify_provenance.py` (5 cases, TDD-first: genuine `workflow_state.py init`
...
- **Where-embedded:** `DIRECTORY.md` (`validate_run_state.py` row now notes the `--verify-provenance`
  mode + the `PROVENANCE_TARGETS` registry) and `workflow/steps/09-verify.md` (Tier A now runs
  `validate_run_state.py --rundir <rundir> --verify-provenance` alongside the lifecycle gate and
  `verify_pack.py`).
- **Status:** EMBEDDED.
- **Verify:** `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_verify_provenance.py" -q`
  (5 passed).

### CR-058 — stage_supervisor.py — the CATCH watchdog wrapping run_stage (G6/D6) (Phase 2 Task 2.1)
- **Date registered:** 2026-07-10
- **What:** Added `trial-runs/_infrastructure/stage_supervisor.py`, the per-stage CATCH watchdog, and
  ...
  `framework/tests/test_stage_supervisor.py` (TDD-first — the `--selftest` passes as a subprocess;
...
- **Where-embedded:** `.claude/skills/run-stage/SKILL.md` (Native section — the native chain now
  ...
  `logs/STATUS.txt`), `workflow/steps/08-scan.md` (the stage-hang CATCH note — the supervisor records the
...
- **Status:** EMBEDDED.
- **Verify:** `python3 trial-runs/_infrastructure/stage_supervisor.py --selftest` (PASS, 3 cases); and
  `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_stage_supervisor.py" -q`
  (2 passed).
...
- **What:** Fixed the babysitter's HEAL-loop liveness guard, which had an always-true conjunct
  ...
  Test: `framework/tests/test_scan_babysitter_livehung.py` (TDD-first — fractional-dm short+full detection;
...
- **Where-embedded:** `workflow/steps/08-scan.md` (the stage-hang CATCH backstop note — the HEAL loop's
  ...
  exists).
- **Status:** EMBEDDED.
- **Verify:** `python3 trial-runs/_infrastructure/scan_babysitter.py --selftest` (PASS, 5 checks); and
  `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_scan_babysitter_livehung.py" -q`
  (2 passed).

### CR-060 — progress_reporter.py + the ~30-min ScheduleWakeup mandate (Phase 2 Task 2.3)
- **Date registered:** 2026-07-10
- **What:** New read-only tool `trial-runs/_infrastructure/progress_reporter.py` — emits ONE progress
  ...
  Test: `framework/tests/test_progress_reporter.py` (TDD-first — the `--selftest` exit-0 case + a
...
- **Where-embedded:** `workflow/steps/08-scan.md` ("Long-compute self-report (G7)" paragraph, beside the
...
- **Status:** EMBEDDED.
- **Verify:** `python3 trial-runs/_infrastructure/progress_reporter.py --selftest` (PASS, 1 case); and
  `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_progress_reporter.py" -q`
  (2 passed).

### CR-061 — stop_dispatch.py + the D18 umbrella branch + the Stop-hook shim (Phase 2 Task 2.4)
- **Date registered:** 2026-07-10
- **What:** New Stop-hook dispatcher `trial-runs/_infrastructure/stop_dispatch.py` — the one Stop-hook
  ...
  Test: `framework/tests/test_stop_dispatch.py` (TDD-first — `--selftest` exit 0, D18 blocks a delivery
...
- **Where-embedded:** `workflow/steps/09-verify.md` (the "D18 umbrella (Stop dispatcher)" paragraph beside
  the scripted Tier-A gates, naming the `python3 trial-runs/_infrastructure/validate_run_state.py --rundir
...
- **Status:** EMBEDDED.
- **Verify:** `python3 trial-runs/_infrastructure/stop_dispatch.py --selftest` (PASS) and
  `bash -n .claude/hooks/stop-dispatcher.sh`; and
  `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_stop_dispatch.py" -q`
  (3 passed).

### CR-062 — stop_dispatch.py CATCH/D6 branch — BLOCK turn-end on unhandled failure records (Phase 2 Task 2.5)
- **Date registered:** 2026-07-10
- **What:** Second Stop-hook branch `branch_catch` in `trial-runs/_infrastructure/stop_dispatch.py`
  ...
  Test: `framework/tests/test_stop_dispatch_catch.py` (TDD-first — CATCH blocks a rundir with an `open`
...
- **Why:** G6/D6 CATCH enforcement — Task 2.1's supervisor already observes and records a hung/failed
  ...
  many branches).
- **Where-embedded:** `workflow/steps/08-scan.md` (the "Stage-hang CATCH (G6/D6)" paragraph now
...
- **Status:** EMBEDDED.
- **Verify:** `python3 trial-runs/_infrastructure/stop_dispatch.py --selftest` (PASS) and
  `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_stop_dispatch_catch.py" -q`
  (2 passed).

### CR-063 — stop_dispatch.py phantom-background branch — BLOCK a turn that claims a job it never launched (Phase 2 Task 2.6)
- **Date registered:** 2026-07-10
- **What:** Third Stop-hook branch `branch_phantom` in `trial-runs/_infrastructure/stop_dispatch.py`
  ...
  CATCH pairs.
  Test: `framework/tests/test_stop_dispatch_phantom.py` (TDD-first — a claim with no `compute_launched`
...
- **Where-embedded:** `workflow/steps/08-scan.md` (new "Phantom-background BLOCK (G5/D5)" paragraph
...
- **Status:** EMBEDDED.
- **Verify:** `python3 trial-runs/_infrastructure/stop_dispatch.py --selftest` (PASS) and
  `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_stop_dispatch_phantom.py" -q`
  (3 passed).
...
- **Date registered:** 2026-07-10
- **What:** Fourth Stop-hook branch `branch_drive` in `trial-runs/_infrastructure/stop_dispatch.py`
  ...
  **and** no live/recent background job exists — reusing the Task 2.6 `_bg_liveness` helper with a
  ...
  Test: `framework/tests/test_stop_dispatch_drive.py` (TDD-first — a pending `next_required` on a
...
- **Where-embedded:** `workflow/steps/03-generate.md` (new "Self-drive at step end (DRIVE/D4)"
  ...
  `workflow_state.py next` non-hook FALLBACK) and `workflow/WORKFLOW.md` (the `workflow_state.py` helper
  row now names `next` as the step-end self-drive check and ties it to the DRIVE/D4 Stop branch).
- **Status:** EMBEDDED.
- **Verify:** `python3 trial-runs/_infrastructure/stop_dispatch.py --selftest` (PASS) and
  `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_stop_dispatch_drive.py" -q`
  (4 passed).
...
- **What:** Fifth Stop-hook branch `branch_skill_coverage` in
  `trial-runs/_infrastructure/stop_dispatch.py` (registered in `BRANCHES` immediately after `drive`).
  ...
  **2**, stderr = the SKILL-COVERAGE/G2 reason fed back to the agent, naming the stage + the missing
  ...
  `framework/tests/test_stop_dispatch_skill.py` (TDD-first, 8 cases on production-shaped
...
- **Why:** G-tier skill-coverage enforcement (G2) — governing skills are mandatory
  ...
  existing dispatcher rather than adding a second Stop hook (one brain, many branches).
- **Where-embedded:** `workflow/WORKFLOW.md` (the "Required skill per stage (SKILL-COVERAGE/G2)"
...
- **Status:** EMBEDDED.
- **Verify:** `python3 trial-runs/_infrastructure/stop_dispatch.py --selftest` (PASS) and
  `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_stop_dispatch_skill.py" -q`
  (8 passed).
...
- **Date registered:** 2026-07-10
- **What:** Sixth Stop-hook branch `branch_detach` in `trial-runs/_infrastructure/stop_dispatch.py`.
  ...
  `framework/tests/test_stop_dispatch_detach.py` (TDD-first, 3 cases via `--branch detach` — an
...
- **Where-embedded:** `workflow/steps/03-generate.md` (new "Detached background jobs (DETACH/N6)"
...
- **Status:** EMBEDDED.
- **Verify:** `python3 trial-runs/_infrastructure/stop_dispatch.py --selftest` (PASS) and
  `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_stop_dispatch_detach.py" -q`
  (3 passed).
...
- **What:** New `.claude/hooks/userpromptsubmit-router.sh` — the L3+L4 (DRIVE/CATCH phase) prompt
  ...
  `trial-runs/_infrastructure/route_prompt.py --prompt "$prompt" --print` (`2>&1` so the
  ...
  and exits 0. The reminder points the executing agent at `workflow/INITIATE.md` → the `physicist-intake`
  ...
  read-only except the best-effort ledger write. Test: `framework/tests/test_userpromptsubmit_router.py`
...
- **Where-embedded:** `workflow/INITIATE.md` (new blockquote under "AGENT EXECUTING THIS REQUEST —
...
- **Status:** EMBEDDED.
- **Verify:** `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_userpromptsubmit_router.py" -q` (3 passed).
...
- **What:** New `.claude/hooks/pretooluse-skill.sh` — the L3+L4 (DRIVE/CATCH phase) skill-precedence
  ...
  read-only. Test: `framework/tests/test_pretooluse_skill.py` (TDD-first, 7 cases: `bash -n` syntax,
...
- **Where-embedded:** `.claude/skills/physicist-intake/SKILL.md` (new "Enforced (G22/N1)" blockquote —
  mirrored to `.agents/skills/` via `sync_skills.py`) and `workflow/INITIATE.md` (new PreToolUse Skill
...
- **Status:** EMBEDDED.
- **Verify:** `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_pretooluse_skill.py" -q` (7 passed).
...
- **What:** The worktree `.claude/settings.json` now wires the complete workflow-adherence hook spine.
  ...
  duplicate blocks). Test (TDD-first): `framework/tests/test_settings_wiring.py` (3 cases: valid
...
- **Where-embedded:** `workflow/WORKFLOW.md` gains a "Mechanized enforcement (hooks)" section listing
  ...
  cite the test.
- **Status:** EMBEDDED.
- **Verify:** `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_settings_wiring.py" -q` (3 passed).
...
- **Verify:** `python3 trial-runs/_infrastructure/check_agent_surface.py` exits 0; `python3 -c "import json;h=json.load(open('.claude/settings.json'))['hooks'];assert 'spk1' not in json.dumps(h)"`

### CR-071 — figure_target `primary` subcommand + single-primary invariant (D9)
- **Date registered:** 2026-07-10
- **What:** added a `primary` subcommand to `trial-runs/_infrastructure/figure_target.py` that owns
  ...
  `framework/tests/test_figure_target_primary.py` (2 cases: the two-primary bug repro → single
...
- **Status:** EMBEDDED.
- **Verify:** `cd /tmp && python3 -m pytest "$REPO/framework/tests/test_figure_target_primary.py::test_primary_enforces_single_primary" -q`

### CR-072 — figure_target `checkin` lifecycle step (D5/D9)
- **Date registered:** 2026-07-10
- **What:** added a `checkin` subcommand to `trial-runs/_infrastructure/figure_target.py` — the
  ...
  `framework/tests/test_figure_target_primary.py` (2 cases: `checkin` flips the flag True on a
...
- **Status:** EMBEDDED.
- **Verify:** `cd /tmp && python3 -m pytest "$REPO/framework/tests/test_figure_target_primary.py::test_checkin_marks_declared_at_checkin_on_primary" "$REPO/framework/tests/test_figure_target_primary.py::test_checkin_refuses_without_a_single_primary" -q`

### CR-073 — fulfil-primary writes verified_by_physicist (D5)
- **Date registered:** 2026-07-10
- **What:** added a `fulfil-primary` subcommand to `trial-runs/_infrastructure/figure_target.py` — the
  ...
  fail-loud, additive (no existing subcommand touched). Test (TDD-first):
  `framework/tests/test_figure_target_primary.py::test_fulfil_primary_writes_verified_by_physicist`
...
- **Status:** EMBEDDED.
- **Verify:** `cd /tmp && python3 -m pytest "$REPO/framework/tests/test_figure_target_primary.py::test_fulfil_primary_writes_verified_by_physicist" -q`

### CR-074 — primary-aware compose (D5)
- **Date registered:** 2026-07-10
- **What:** `trial-runs/_infrastructure/figure_target.py compose`'s `--figure-id` is now OPTIONAL.
  ...
  `framework/tests/test_figure_target_primary.py`:
...
- **Where-embedded:** `figure_target.py`'s module-docstring Usage line now reads
  ...
  convenience).
- **Status:** EMBEDDED.
- **Verify:** `cd /tmp && python3 -m pytest "$REPO/framework/tests/test_figure_target_primary.py" -q`

### CR-075 — primary-aware inv_figure_contract_fulfilled (D9/G10)
- **Date registered:** 2026-07-10
- **What:** `trial-runs/_infrastructure/validate_run_state.py`'s `inv_figure_contract_fulfilled`
...
- **Where-embedded:** `workflow/checklists/figure-contract.md` Emission-contract section (the new
  `validate_run_state.py` PRIMARY-aware hard-FAIL bullet).
- **Status:** EMBEDDED.
- **Verify:** `cd /tmp && python3 -m pytest "$REPO/framework/tests/test_validate_run_state.py::test_primary_missing_side_by_side_fails_in_non_reproduce_mode" -q`
  and `cd /tmp && python3 "$REPO/trial-runs/_infrastructure/validate_run_state.py" --selftest` (8 cases).

### CR-076 — verify_pack primary-aware FAIL (D9/G10)
- **Date registered:** 2026-07-10
- **What:** `trial-runs/_infrastructure/verify_pack.py`'s `check_figure_target` (the shared checker for
  ...
  (TDD-first), `framework/tests/test_verify_pack_primary.py`:
...
- **Where-embedded:** `workflow/steps/09-verify.md`'s `verify_pack.py` invocation comment now reads
  `figure-contract PRIMARY target fulfilled-or-FAILed (non-primary WARNed)`.
- **Status:** EMBEDDED.
- **Verify:** `cd /tmp && python3 -m pytest "$REPO/framework/tests/test_verify_pack_primary.py" -q` (3 cases).

### CR-077 — validate_parameters.py + validations.json (D10)
- **Date registered:** 2026-07-10
- **What:** new stdlib-only tool `trial-runs/_infrastructure/validate_parameters.py` (Phase 3, G12)
  ...
  TDD-first test `framework/tests/test_validate_parameters.py` (3 cases: subprocess `--selftest`,
...
- **Status:** EMBEDDED.
- **Verify:** `cd /tmp && python3 "$REPO/trial-runs/_infrastructure/validate_parameters.py" --selftest`
  (5 cases) and `cd /tmp && python3 -m pytest "$REPO/framework/tests/test_validate_parameters.py" -q` (3 cases).

### CR-078 — inv_param_validated_before_scan (D10/G12)
- **Date registered:** 2026-07-10
- **What:** new invariant `inv_param_validated_before_scan` registered inside
  `trial-runs/_infrastructure/validate_run_state.py`'s `INVARIANTS` tuple (keyed to the `analysis`
  ...
  `framework/tests/test_validate_run_state.py::test_scan_with_pending_param_validation_fails` (PENDING →
...
- **Where-embedded:** `.claude/skills/run-scan/SKILL.md` (new "## 1b. Validate the varied params BEFORE
  launch" gate; re-mirrored to `.agents/skills/` via `sync_skills.py`) and `workflow/steps/08-scan.md`
...
- **Status:** EMBEDDED.
- **Verify:** `cd /tmp && python3 -m pytest "$REPO/framework/tests/test_validate_run_state.py::test_scan_with_pending_param_validation_fails" -q`;
  `cd /tmp && python3 "$REPO/trial-runs/_infrastructure/validate_run_state.py" --selftest` (9 cases);
  `python3 "$REPO/trial-runs/_infrastructure/validate_run_state.py" --rundir "$REPO/trial-runs/sleptonscan_fig3_SCAN"` (exit 0).

### CR-079 — deviations-on-change broadened to baselined-input edits (D15)
- **Date registered:** 2026-07-10
- **What:** broadened `inv_deviations_on_change` in `trial-runs/_infrastructure/validate_run_state.py`
...
- **Why:** D15/G17 (post-hoc half). CHECK-IN 1 freezes the approved plan, but nothing forced a
  ...
  still FAILs.
- **Where-embedded:** `workflow/checklists/check-ins.md` (DEVIATION CHECK-INS section — a new note that
...
- **Status:** EMBEDDED.
- **Verify:** `cd /tmp && python3 -m pytest "$REPO/framework/tests/test_validate_run_state.py::test_baselined_input_edit_without_deviation_row_fails" "$REPO/framework/tests/test_validate_run_state.py::test_deviations_invariant_unchanged_without_run_state" -q`;
  `cd /tmp && python3 "$REPO/trial-runs/_infrastructure/validate_run_state.py" --selftest` (10 cases).
...
- **What:** the moment-of-change twin of CR-079. New `edit_guard(changed_path)` in
  `trial-runs/_infrastructure/validate_run_state.py` (returns 0 allow / 1 block: 1 iff `changed_path`
...
- **Why:** D15/G17. CR-079's `DEVIATIONS-on-change` invariant catches an undocumented baselined-input
  ...
  checker.
- **Where-embedded:** `workflow/checklists/check-ins.md` (DEVIATION CHECK-INS — the baselined-input note
...
- **Status:** EMBEDDED.
- **Verify:** `cd /tmp && python3 -m pytest "$REPO/framework/tests/test_deviations_guard.py" -q`;
  `cd /tmp && python3 "$REPO/trial-runs/_infrastructure/validate_run_state.py" --selftest` (11 cases).
...
- **What:** new invariant `inv_ladder_order` registered inside
  `trial-runs/_infrastructure/validate_run_state.py`'s `INVARIANTS` tuple as the `ladder-order`
  ...
  `framework/tests/test_validate_run_state_lifecycle.py::test_ladder_order_fails_without_smoke_rung`
...
- **Where-embedded:** `workflow/steps/08-scan.md` (new "Record the smoke rung before the full scan
  (D11/G13)" note after the cost_preflight disk-budget block — names the `logs/ladder.json` schema and
  the `ladder-order` FAIL); `workflow/checklists/verification-panel.md` (Tier-A `validate_run_state.py`
  invariant list now reads "R5/pairing/ladder-order" and the exit-0 bullet spells out the smoke-rung
  requirement).
- **Status:** EMBEDDED.
- **Verify:** `cd /tmp && python3 -m pytest "$REPO/framework/tests/test_validate_run_state_lifecycle.py::test_ladder_order_fails_without_smoke_rung" -q`;
  `cd /tmp && python3 "$REPO/trial-runs/_infrastructure/validate_run_state.py" --selftest` (exit 0, 12 cases).
...
- **What:** new invariant `inv_certify_before_limit` registered inside
  `trial-runs/_infrastructure/validate_run_state.py`'s `INVARIANTS` tuple as the `certify-before-limit`
  ...
  `framework/tests/test_validate_run_state_lifecycle.py::{test_certify_before_limit_fails_on_fail_cert,
  ...
  (`framework/tests/test_validate_run_state.py`) — a complete scan aggregator that ships a
...
- **Where-embedded:** `workflow/steps/07-exclude.md` ("Make the limit trustworthy" section — new
  **Enforced (D12/G14)** note spelling out that `certify-before-limit` hard-blocks a FAIL/absent cert and
  that scan attestation does not waive it); `workflow/checklists/verification-panel.md` (Tier-A
...
- **Status:** EMBEDDED.
- **Verify:** `cd /tmp && python3 -m pytest "$REPO/framework/tests/test_validate_run_state_lifecycle.py::test_certify_before_limit_fails_on_fail_cert" "$REPO/framework/tests/test_validate_run_state_lifecycle.py::test_certify_before_limit_scan_attestation_insufficient" -q`;
  `cd /tmp && python3 "$REPO/trial-runs/_infrastructure/validate_run_state.py" --selftest` (exit 0, 13 cases).
...
- **What:** new invariant `inv_trap_obligations` registered inside
  `trial-runs/_infrastructure/validate_run_state.py`'s `INVARIANTS` tuple as the
  ...
  `framework/tests/test_validate_run_state_lifecycle.py::test_trap_obligations_pending_blocks` (T8 PENDING
...
- **Where-embedded:** `workflow/checklists/physics-traps.md` (Sweep output section — the `obligations[]`
  ...
  `sync_skills.py`.
- **Status:** EMBEDDED.
- **Verify:** `cd /tmp && python3 -m pytest "$REPO/framework/tests/test_validate_run_state_lifecycle.py" "$REPO/framework/tests/test_validate_run_state.py" -q` (25 passed);
  `cd "$REPO" && python3 trial-runs/_infrastructure/validate_run_state.py --selftest` (exit 0, 14 cases).

### CR-084 — `sr_plausibility.py`: the D14 analysis→statistics plausibility EMITTER (new stdlib tool)
- **Date registered:** 2026-07-10
- **What:** new stdlib-only tool `trial-runs/_infrastructure/sr_plausibility.py` (Phase 4, L5b). It
...
- **Where-embedded:** `workflow/steps/07-exclude.md` (a **Plausibility emitter (D14)** note after the
...
- **Status:** EMBEDDED.
- **Verify:** `python3 trial-runs/_infrastructure/sr_plausibility.py --selftest` (exit 0, 3 cases);
  `cd /tmp && python3 -m pytest "$REPO/framework/tests/test_sr_plausibility.py" -q` (3 passed).
...
- **Where-embedded:** `workflow/steps/07-exclude.md` (a **Plausibility FOLD (D14, enforced)** note after
...
- **Status:** EMBEDDED.
- **Verify:** `python3 trial-runs/_infrastructure/validate_run_state.py --selftest` (exit 0, 15 cases);
  `cd /tmp && python3 -m pytest "$REPO/framework/tests/test_validate_run_state_lifecycle.py::test_implausible_sr_plausibility_fails_statistics" -q` (1 passed).
...
- **Where-embedded:** `workflow/steps/08-scan.md` (a **Keep every point's evidence IN-TREE (N2/G23)** note
...
- **Status:** EMBEDDED.
- **Verify:** `python3 trial-runs/_infrastructure/validate_run_state.py --selftest` (exit 0, 16 cases);
  `cd /tmp && python3 -m pytest "$REPO/framework/tests/test_validate_run_state_lifecycle.py::test_outputs_in_tree_fails_on_tmp_point" -q` (1 passed).
...
- **Why:** N4, L5b physics-lifecycle invariant. A downstream stage that showers a half-written LHE —
  ...
  would read it.
- **Where-embedded:** `workflow/steps/03-generate.md` (a **Producer barrier — don't consume a mid-write
...
- **Status:** EMBEDDED.
- **Verify:** `python3 trial-runs/_infrastructure/validate_run_state.py --selftest` (exit 0, 17 cases);
  `cd /tmp && python3 -m pytest "$REPO/framework/tests/test_validate_run_state_lifecycle.py::test_producer_barrier_fails_on_event_count_mismatch" -q` (1 passed).
...
- **Where-embedded:** `workflow/checklists/verification-panel.md` (a Tier A checkbox running
  `validate_run_state.py --rundir <rundir> --verify-provenance` and naming the hand-written/backfilled
  rejection that closes the loophole); the mode is already invoked in `workflow/steps/09-verify.md`.
- **Status:** EMBEDDED.
- **Verify:** `python3 trial-runs/_infrastructure/validate_run_state.py --selftest` (exit 0, 18 cases);
  `cd /tmp && python3 -m pytest "$REPO/framework/tests/test_validate_run_state_lifecycle.py::test_verify_provenance_rejects_handwritten_artifact" -q` (1 passed).
...
- **Where-embedded:** `workflow/checklists/verification-panel.md` (the `--verify-provenance` Tier-A checkbox
  now also names the `input_fingerprint` recompute and the stale-after-inputs-changed rejection);
  `framework/FAILURE-CATALOGUE.md` D4 (backfilled/stale required artifact → caught by `--verify-provenance`
  → guard in `verify_provenance_lifecycle`).
- **Status:** EMBEDDED.
- **Verify:** `python3 trial-runs/_infrastructure/validate_run_state.py --selftest && python3 trial-runs/_infrastructure/sr_plausibility.py --selftest` (both exit 0; validate_run_state prints 19 cases);
  `cd /tmp && python3 -m pytest "$REPO/framework/tests/test_validate_run_state_lifecycle.py" "$REPO/framework/tests/test_sr_plausibility.py" "$REPO/framework/tests/test_validate_run_state.py" -q` (task's own `test_verify_provenance_rejects_fingerprint_mismatch` + lifecycle/sr_plausibility suites pass; one PRE-EXISTING unrelated failure `test_validate_run_state.py::test_scan_aggregator_without_sibling_intermediates_passes_generation_and_analysis` from CR-086's `inv_outputs_in_tree` N2 gate vs a tmp-sibling `run_dir` fixture, present on HEAD before this CR).
...
- **Verify:** `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_validate_run_state.py" -q` (21 passed); `python3 trial-runs/_infrastructure/validate_run_state.py --selftest` (19 cases)
...
- **Where-embedded:** `.claude/skills/resource-sweep/SKILL.md` ("When a STAGE FAILS: `--debug
  recipe-search`" section + the `--selftest` self-check), referenced from `workflow/steps/09-verify.md`
  (the FAIL-verdict path fires the recipe search before improvising a stage-level fix).
- **Status:** EMBEDDED.
- **Verify:** `python3 trial-runs/_infrastructure/resource_census.py --selftest` (exit 0, `PASS (3 case(s))`);
  `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_resource_census_gates.py" -q` (3 passed).
...
- **Where-embedded:** `workflow/steps/09-verify.md` (Tier-A "Recipe-search close-block (G8)" bullet + the
...
- **Status:** EMBEDDED.
- **Verify:** `python3 trial-runs/_infrastructure/resource_census.py --selftest && python3 trial-runs/_infrastructure/stop_dispatch.py --selftest` (both exit 0; resource_census prints `PASS (5 case(s))`, stop_dispatch's `PASS` now includes the `recipe-search` branch case);
  `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_resource_census_gates.py" -q` (5 passed, incl. `test_assert_recipe_search_close_block` + `test_recipe_search_stop_branch`).
...
- **Where-embedded:** CLAUDE.md skills roster (16; added `stage-recovery` + the previously-unlisted
  `resource-sweep`); `DIRECTORY.md` `.claude/skills/` row; referenced from `workflow/steps/03-generate.md`
  (generation-failure bullet) and `workflow/steps/08-scan.md` (Stage-hang CATCH clear-it clause).
- **Status:** EMBEDDED.
- **Verify:** `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_stage_recovery_skill.py" -q`
  (1 passed); `python3 trial-runs/_infrastructure/sync_skills.py && python3 trial-runs/_infrastructure/check_agent_surface.py`
...
- **Verify:** `python3 trial-runs/_infrastructure/resource_census.py --selftest` (exit 0, `PASS (7 case(s))`);
  `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_resource_census_gates.py" -q` (7 passed, incl. `test_assert_pre_generate` + `test_assert_pre_generate_no_model_passes`).
...
- **Where-embedded:** `workflow/steps/03-generate.md` (the gate paragraph); `.claude/settings.json` (the merged
  PostToolUse block); `DIRECTORY.md` (the `pre-generate-guard.sh` row). Closes CR-094's deferred doc-embed.
- **Status:** LANDED.
- **Verify:** `python3 trial-runs/_infrastructure/resource_census.py --selftest` (exit 0, `PASS (8 case(s))`);
  `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_pre_generate_guard.py" "$REPO/framework/tests/test_settings_wiring.py" "$REPO/framework/tests/test_deviations_guard.py" -q` (green — the pre-generate-guard, the observer, and the deviations-guard all coexist in `PostToolUse`).

### CR-096 — `preflight_watcher.py --arm` — completion-watcher FIRE-COMMAND preflight (N3/G24)
- **Date registered:** 2026-07-10
- **What:** new stdlib-only tool `trial-runs/_infrastructure/preflight_watcher.py`. Before a backgrounded
...
- **Where-embedded:** `workflow/steps/08-scan.md` (the DRIVE Watcher-preflight ARM (N3/G24) paragraph);
  `DIRECTORY.md` (the `_infrastructure/` software-track row). Surface coherence re-checked with
  `check_agent_surface.py`.
- **Status:** LANDED.
- **Verify:** `python3 trial-runs/_infrastructure/preflight_watcher.py --selftest` (exit 0,
  `preflight_watcher selftest: PASS (5 case(s))`);
  `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_preflight_watcher.py" -q` (2 passed).
...
- **Where-embedded:** `workflow/steps/08-scan.md` (the DRIVE Watcher-preflight ARM (N3/G24) paragraph now
...
- **Status:** LANDED.
- **Verify:** `python3 trial-runs/_infrastructure/preflight_watcher.py --selftest && python3 trial-runs/_infrastructure/stop_dispatch.py --selftest`
  (exit 0; `preflight_watcher selftest: PASS (7 case(s))`; `stop_dispatch selftest: PASS`);
  `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_preflight_watcher.py" -q` (5 passed).
...
- **Where-embedded:** `workflow/steps/09-verify.md` (Tier A gains the N5/G26 open-defect close-block bullet,
  the `verify_pack.py` block comment lists the open-note check, and the D18 umbrella paragraph names the
  `branch_open_defect` sibling Stop-branch + its FALLBACK), `workflow/checklists/verification-panel.md`
  ...
  `check_agent_surface.py`.
- **Status:** LANDED.
- **Verify:** `python3 trial-runs/_infrastructure/verify_pack.py --selftest && python3 trial-runs/_infrastructure/stop_dispatch.py --selftest`
  (exit 0; `verify_pack selftest: PASS (2 case(s))`; `stop_dispatch selftest: PASS`);
  `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_verify_pack_open_defect.py" -q` (4 passed).

### CR-099 — `validate_checkin.py` CHECK-IN schema validator (G18)
- **Date registered:** 2026-07-10 (spine-hardening Phase 4b Task 4.18).
- **What:** a new stdlib-only validator `trial-runs/_infrastructure/validate_checkin.py` that gates an
  emitted CHECK-IN artifact against `workflow/checklists/check-ins.md`, mirroring
...
- **Where-embedded:** `workflow/checklists/check-ins.md` (the closing "Emit + validate (the machine gate,
...
- **Status:** LANDED.
- **Verify:** `python3 trial-runs/_infrastructure/validate_checkin.py --selftest`
  (exit 0; `validate_checkin selftest: PASS (3 good + 5 bad)`);
  `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_validate_checkin.py" -q` (5 passed).
...
- **What:** a new tracked installer `trial-runs/_infrastructure/install_git_hooks.sh` that writes an
...
- **Status:** LANDED.
- **Verify:** `bash trial-runs/_infrastructure/install_git_hooks.sh` (exit 0; prints the installed hook
  path) and `python3 trial-runs/_infrastructure/check_agent_surface.py` (exit 0);
  `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_precommit_hook.py" -q` (2 passed).
...
- **What:** a new tracked engine `framework/spine_sim/run_spine_sim.py` (stdlib-only, modeled on
  `framework/verify_fixes.py`'s subprocess-board shape) plus its `framework/spine_sim/cases/` seed dir.
...
- **Where-embedded:** `framework/spine_sim/README.md` (one-paragraph orientation); `DIRECTORY.md`
  software rows for `framework/spine_sim/` and `framework/spine_sim/run_spine_sim.py`. Case scripts and
...
- **Verify:** `python3 framework/spine_sim/run_spine_sim.py --selftest` (exit 0, prints
  `run_spine_sim selftest: PASS`) and
  `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_spine_sim.py" -q` (10 passed).
...
- **What:** a new tracked, stdlib-only module `framework/spine_sim/_case_lib.py` that every
...
- **Where-embedded:** `framework/spine_sim/README.md` (the "Case toolkit" paragraph); `DIRECTORY.md`
  software row for `framework/spine_sim/_case_lib.py`.
- **Status:** LANDED (toolkit + test; case scripts import it in the sibling 6.x tasks).
- **Verify:** `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_spine_sim_caselib.py" -q`
...
- **What:** 8 new tracked case scripts under `framework/spine_sim/cases/` — `case_G10.py`, `case_G12.py`,
  `case_G13.py`, `case_G14.py`, `case_G15.py`, `case_G16.py`, `case_G23.py`, `case_G25.py` — plus the
  integration test `framework/tests/test_spine_sim_invariants.py`. Each imports `_case_lib`, seeds a
...
- **Where-embedded:** `framework/spine_sim/README.md` (new "Case coverage" table, 8 rows);
  `DIRECTORY.md` `framework/spine_sim/cases/` folder row listing the 8 case files + their enforcement.
- **Status:** LANDED (8 cases + integration test; all fire against the as-built enforcement).
- **Verify:** `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_spine_sim_invariants.py" -q`
  (1 passed) and `python3 framework/spine_sim/run_spine_sim.py --only G10,G12,G13,G14,G15,G16,G23,G25`
...
- **What:** 7 new tracked case scripts under `framework/spine_sim/cases/` — `case_G2.py`, `case_G4.py`,
  `case_G5.py`, `case_G6.py`, `case_G8.py`, `case_G11.py`, `case_G27.py` — plus the integration test
  `framework/tests/test_spine_sim_stop.py`. Each imports `_case_lib`, seeds a throwaway rundir, and
...
- **Where-embedded:** `framework/spine_sim/README.md` (the "Case coverage" table gains 7 rows and its
  intro/column now name the Stop-dispatch + watchdog lane); `DIRECTORY.md` `framework/spine_sim/cases/`
...
- **Verify:** `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_spine_sim_stop.py" -q`
  (1 passed) and `python3 framework/spine_sim/run_spine_sim.py --only G2,G4,G5,G6,G8,G11,G27`
...
- **What:** 14 new tracked case scripts under `framework/spine_sim/cases/` — `case_G0a.py`,
  ...
  integration test `framework/tests/test_spine_sim_hooks_tools.py`. This is the THIRD `spine_sim`
  integration lane, spanning Phases 0–4: the Phase-0 spike re-checks (G0a–G0c re-run
  `framework/spine/spike_probe.py --spike SPK-{1,2,3} --check` on the recorded artifacts); the
...
- **Where-embedded:** `framework/spine_sim/README.md` (the "Case coverage" table gains 14 rows; the
  intro now names the hook + tool lane as the third integration lane and its direct-drive discipline);
  `DIRECTORY.md` `framework/spine_sim/cases/` folder row listing the 14 case files + their
  hook/tool/branch.
- **Status:** LANDED (14 cases + integration test; all fire against the as-built enforcement).
- **Verify:** `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_spine_sim_hooks_tools.py" -q`
  (1 passed) and `python3 framework/spine_sim/run_spine_sim.py --only G0a,G0b,G0c,G1,G3,G7,G9,G17,G18,G19,G20,G22,G24,G26`
...
- **What:** `framework/spine_sim/clean_room.py` (stdlib-only, the D17 un-hinted self-drive launcher) +
  `framework/spine_sim/cases/case_G21.py` + `framework/spine_sim/self_drive/.gitkeep` + the test
  `framework/tests/test_clean_room.py` (6). `clean_room.py` builds an **un-hinted** launch command
...
- **As-built / EXECUTION ADJUSTMENT (auth finding):** headless `claude -p` is **NOT authenticated in
  ...
  **SKIP by default** (never faked to PASS — running `--live` unauthenticated would write a FAIL
  ...
  tracked via `.gitkeep`.
- **Where-embedded:** `framework/spine_sim/README.md` (self-drive lane paragraph + G21 coverage row),
  `workflow/steps/09-verify.md` (the "enforcement spine is itself verified (L6)" note),
...
- **Verify:** `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_clean_room.py" -q`
  (6 passed), `python3 framework/spine_sim/clean_room.py --selftest` (exit 0), and
  `python3 framework/spine_sim/run_spine_sim.py --require-all` (29 PASS / 1 SKIP / 0 FAIL, exit 0).
...
- **What:** `framework/green_board.py` (stdlib-only) + the repo-root `Makefile` (`green` /
  `green-self-drive` / `spine-sim` targets) + the test `framework/tests/test_green_board.py` (5).
...
- **Where-embedded:** `workflow/WORKFLOW.md` (the "one aggregate green bar" note under Mechanized
  enforcement), `DIRECTORY.md` (rows for `framework/green_board.py` + the root `Makefile`).
...
- **Verify:** `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_green_board.py" -q`
  (5 passed), and `python3 framework/green_board.py --json` lists the four stack rungs.
...
- **What:** the whole-spine completeness gate `framework/tests/test_spine_sim_complete.py` (2 tests) —
  ...
  Plus the verification RECORD: `framework/spine_sim/README.md` finalized to carry the sorted 30-row
  coverage table (gate | trigger | mechanism | case | GREEN assertion); `PRODUCT-CONTRACT.md` §9 scope
  ...
  make-green resolution CR-107 explicitly deferred here: `framework/green_board.py` swaps the red legacy
  ...
  plus the informational audit; `framework/tests/test_green_board.py`'s default-rung assertion updated
  to match.
...
- **Where-embedded:** `framework/spine_sim/README.md` (the 30-row coverage table + completeness-gate
  lead-in), `PRODUCT-CONTRACT.md` (§9 verification-scope row), `framework/FAILURE-CATALOGUE.md` (the D4
...
- **Verify:** `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_spine_sim_complete.py" -q`
...
- **What:** the directory-keeper reconcile that closes the map against the landed spine + its regression
  guard. `framework/tests/test_directory_reconciled.py` (2 tests) pins the directory-keeper contract:
  ...
  `framework/spine_sim/`, and `framework/spine/HOOK-PRIMACY.json` — carries a DIRECTORY.md row, and the
...
- **Where-embedded:** `DIRECTORY.md` (the `.claude/settings.json` row rewrite + the `docs/superpowers/`
  row), `trial-runs/README.md` (per-run layout), `framework/tests/test_directory_reconciled.py`.
- **Status:** LANDED (2026-07-10) — `check_agent_surface.py` exit 0 with `[PASS] dirmap` + `[PASS] refs`
  and no WARN.
- **Verify:** `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_directory_reconciled.py" -q && cd "$REPO" && python3 trial-runs/_infrastructure/check_agent_surface.py`
...
- **What:** embed the workflow-adherence spine's failure classes into `framework/FAILURE-CATALOGUE.md` as
  ...
  **G27**. `framework/tests/test_failure_catalogue_spine.py` (3 tests) is the durable regression: every
...
- **Why:** the FAILURE-CATALOGUE is the durable records-track memory + the Tier-B attack list + a seed for
  ...
  non-droppable.
- **Where-embedded:** `framework/FAILURE-CATALOGUE.md` (the D4–D19 + N1–N6 entries + the extended Tier-B
  paragraph), `framework/tests/test_failure_catalogue_spine.py`.
- **Status:** LANDED (2026-07-10) — the spine test is 3 PASS; the class↔gate map matches
  `framework/spine_sim/README.md` (G0a–G27) and the landed `stop_dispatch.py`/`validate_run_state.py`
  guards.
- **Verify:** `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_failure_catalogue_spine.py" -q` (3 passed).
...
- **Where-embedded:** PRODUCT-CONTRACT.md §6/§7; `framework/tests/test_product_contract_policies.py`
  (4 tests: the three policy rows + the enum-mirror guard).
- **Status:** EMBEDDED (2026-07-10) — 4 PASS; `check_agent_surface.py` green.
- **Verify:** `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_product_contract_policies.py" -q` (4 passed).
...
- **What:** append a `## Skill-trigger behavior evals` section to `framework/ROUTING-EVALS.md` — a P4-style
...
- **Where-embedded:** `framework/ROUTING-EVALS.md` (the new section); `framework/tests/test_routing_evals_skill_triggers.py`
...
- **Verify:** `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_routing_evals_skill_triggers.py" -q` (2 passed).
...
- **What:** the closing green-board sweep — `framework/tests/test_records_reconciled.py` provides the
  ...
  (`grep -rIE '<tool>' workflow/ .claude/` ≥1 hit for each — `spike_probe.py` is the sole 0-hit and
...
- **Where-embedded:** `framework/tests/test_records_reconciled.py`; the board commands above;
  `workflow/WORKFLOW.md` + `workflow/steps/09-verify.md` (the aggregate `green_board.py`/`make green`
  board); `Makefile` (`make green`).
- **Status:** EMBEDDED (2026-07-10) — 2 PASS.
- **Verify:** `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_records_reconciled.py" -q && cd "$REPO" && python3 trial-runs/_infrastructure/check_agent_surface.py && python3 framework/green_board.py`

### CR-114 — G21 clean-room self-drive: live PASS (authenticated subagent)
- **Date registered:** 2026-07-10
- **What:** ran the clean-room self-drive (G21/D17) with an authenticated in-harness subagent (claude -p auth-blocked per SPK-1). A fresh un-hinted agent given only a physics request read workflow/INITIATE.md FIRST, loaded physicist-intake FIRST (no repo survey), produced a validated task_contract.json, and stopped at CHECK-IN 1 (validate_checkin.py passing) with run_state.json recording route+skill and the run-dir locked — zero compute, BLOCKED awaiting approval. Recorded framework/spine_sim/self_drive/last_verdict.json (verdict=PASS) + the curated evidence run dir.
...
- **Verify:** `python3 framework/spine_sim/cases/case_G21.py` exit 0; `python3 framework/spine_sim/run_spine_sim.py --require-all` -> 30 PASS
...
- **What:** the curated-export dry-run (`export_distribution.sh … --allow-placeholder-license`) failed
  ...
  from the distributable stage (`.claude/hooks/`, `framework/tests/`, and the spine_sim/green_board
  verification harness). Reworded ONLY those dead references to describe each by its role instead of a
  backtick repo path, keeping the enforcement description intact: in `workflow/WORKFLOW.md` the five
  hook scripts became "the PreToolUse card-guard / the skill-precedence guard / the PostToolUse
  observer / the UserPromptSubmit router / the Stop dispatcher", `framework/tests/test_settings_wiring.py`
  → "the settings-wiring test (dev-repo/CI only)", `framework/green_board.py` → "the `make green`
  aggregate board", `framework/tests/test_spine_sim_complete.py` → "the spine_sim completeness test
  (dev-repo, CR-108)"; in `workflow/steps/09-verify.md` `framework/spine_sim/run_spine_sim.py` → "the
  spine_sim verification board (run on-demand in the dev repo)" and `framework/spine_sim/self_drive/` →
...
- **Why:** a shipped doc must not name, as a must-exist backtick path, a file the distribution
  ...
  each guard enforces.
- **Where-embedded:** `workflow/WORKFLOW.md` (hook table + the L6 aggregate-board / settings-wiring /
  spine_sim-completeness lines); `workflow/steps/09-verify.md` (the L6 spine-verification section).
- **Status:** EMBEDDED (2026-07-11).
- **Verify:** `rm -rf /tmp/hep-dist && bash trial-runs/_infrastructure/export_distribution.sh /tmp/hep-dist --allow-placeholder-license` exits 0 (gate 4c clean, evidence gate 14 PASS, size guard clean); `python3 trial-runs/_infrastructure/check_agent_surface.py` exit 0; `python3 framework/green_board.py` exit 0.

### CR-116 — lhe_check JSON sidecar (default-on) + `lhe-check-before-shower` invariant (A1)
- **Date registered:** 2026-07-11
- **What:** `trial-runs/_infrastructure/lhe_check.py` now ALWAYS writes a JSON sidecar (default
...
- **Where-embedded:** `workflow/steps/03-generate.md` (pre-shower guard paragraph: sidecar
  default-on + gated); `.claude/skills/run-stage/SKILL.md` step 4 (+ `.agents/skills/` mirror via
  `sync_skills.py`); `framework/tests/test_lhe_check_gate.py` (10 tests: emitter sidecar/verdict/
  `--json-out`, invariant FAIL/PASS/legacy-waiver/no-defaulting, registration + selftest).
- **Status:** EMBEDDED (2026-07-11).
- **Verify:** `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_lhe_check_gate.py" "$REPO/framework/tests/test_validate_run_state.py" -q && cd "$REPO" && python3 trial-runs/_infrastructure/validate_run_state.py --selftest` (prints `20 case(s)`).
...
- **Verify:** `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_figure_target_primary.py" -q` (14 passed) and `python3 trial-runs/_infrastructure/validate_run_state.py --selftest` (21 cases)
...
- **Verify:** `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_pretooluse_skill.py" "$REPO/framework/tests/test_userpromptsubmit_router.py" "$REPO/framework/tests/test_settings_wiring.py" "$REPO/framework/tests/test_resource_census_gates.py" "$REPO/framework/tests/test_stop_dispatch_skill.py" -q` (37 passed)
...
- **Verify:** `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_validate_run_state_lifecycle.py" -q` (14 passed) and `python3 trial-runs/_infrastructure/validate_run_state.py --selftest` (22 cases)
...
- **Verify:** `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_validate_checkin.py" -q` (9 passed)
...
- **Verify:** `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_failure_catalogue_spine.py" -q`
...
- **Verify:** `grep -c "TRIAL-ADHERENCE-SELF-AUDIT" trial-runs/2026-07-06_*SVJ*/RESULT.md` ≥1 and
  ...
  `grep "SVJ-tchannel" DIRECTORY.md | grep -c "zero compute"` = 0; `python3 trial-runs/_infrastructure/check_agent_surface.py` exit 0 (no logs WARN).
- **Status:** EMBEDDED.
...
- **Verify:** `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_cost_preflight_artifact.py" -q`
...
- **Verify:** `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_approval_chain.py" -q` (15 passed incl. settings wiring)
...
- **Verify:** `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_stop_dispatch_drift.py" -q`
...
- **Verify:** `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_protect_enforcement.py" -q`
...
- **Verify:** `REPO="$(pwd)" && cd /tmp && python3 -m pytest "$REPO/framework/tests/test_sessionstart_orient.py" -q`
...
rebase provenance), `framework/benchmark/BENCHMARK.md` (dated re-lock history), and the
`framework/STATUS.md` session log. Backfill an entry here only when a pre-registry change is
touched again.
...
- **What:** when hepdata.net bulk endpoints are blocked server-side (403/robot-check), harvest via
  ...
  workflow/checklists/data-acquisition.md (browser-route recipe block).
- **Status:** EMBEDDED.
...
- **What:** identity/spot-check rows shown at any check-in use the EXACT harvested values wherever a
  ...
  **Why:** catalogue A9 — CHECK-IN 2 quoted a 2.5%-off survey read-off while the exact HEPData value
  sat in outputs/hepdata/. **Where-embedded:** workflow/checklists/summary-plot.md §3 (identity
  checks) + §4 (waypoint).
- **Status:** EMBEDDED.
