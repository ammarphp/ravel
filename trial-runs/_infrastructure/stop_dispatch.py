...
def branch_d18(ctx):
    ...
    validator = [sys.executable,
                 os.path.join(ctx["repo"], "trial-runs/_infrastructure/validate_run_state.py")]
    try:
...
def branch_recipe_search(ctx):
    ...
    census = os.path.join(ctx["repo"], "trial-runs/_infrastructure/resource_census.py")
    try:
...
def branch_stage_drift(ctx):
    ...
    if disk > cursor_idx:
        return True, (f"STAGE-DRIFT: the disk shows the run at '{stage_name}' but "
                      ...
                      f"`python3 trial-runs/_infrastructure/workflow_state.py advance --rundir "
...
def branch_armed_watcher(ctx):
    ...
    Non-invariant -> wired here as its own branch, not under the D18 umbrella."""
    pf = os.path.join(ctx["repo"], "trial-runs/_infrastructure/preflight_watcher.py")
    try:
...
def branch_open_defect(ctx):
    ...
    if not ctx.get("is_delivery"):
        return False, ""
    vp = os.path.join(ctx["repo"], "trial-runs/_infrastructure/verify_pack.py")
    try:
