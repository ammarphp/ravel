...
.claude/rules/madgraph-pythia.md and workflow/checklists/model-cards.md):
  ...
  * MULTIWEIGHT (<rwgt>/<wgt> tags): single-weight pipelines must run
    `rivet --skip-weights` and generate with use_syst=False (workflow/steps/04-analyze.md).
...
def main():
    ...
    if multiweight:
        note("WARN", "multiweight",
             ...
             "with use_syst=False) — see workflow/steps/04-analyze.md")
    else:
