...
See workflow/steps/05-visualize.md and .claude/rules/plots.md. Run in the `recast` env (ROOT 6.40).
"""
import argparse
import json
import math
...
def atlas_style():
    ...
    """
    st = ROOT.TStyle("ATLASmirror", "ATLAS-like house style (ROOT mirror)")
    ...
    ROOT.gROOT.SetStyle("ATLASmirror")
...
def cmd_yields(args):
    ...
    atlas_style()
    c = ROOT.TCanvas("c_yields", "per-SR yields (ROOT mirror)", 900, 700)
    if use_logy:
        c.SetLogy(True)
    ...
    h_bkg = ROOT.TH1F("h_bkg", "", nsr, 0.0, float(nsr))
    h_sig = ROOT.TH1F("h_sig", "", nsr, 0.0, float(nsr))   # signal stacked ON TOP of background
    for i in range(nsr):
    ...
    h_bkg.SetFillColor(ROOT.TColor.GetColorTransparent(tcolor("#999999"), 0.85))
    ...
    h_band = ROOT.TH1F("h_band", "", nsr, 0.0, float(nsr))
    for i in range(nsr):
    ...
    gx = ROOT.std.vector("double")()
    gy = ROOT.std.vector("double")()
    gex = ROOT.std.vector("double")()
    gey = ROOT.std.vector("double")()
    for i in range(nsr):
        gx.push_back(i + 0.5)
        gy.push_back(n_obs[i])
        gex.push_back(0.0)
        gey.push_back(n_obs[i] ** 0.5)
    g_data = ROOT.TGraphErrors(nsr, gx.data(), gy.data(), gex.data(), gey.data())
    ...
    h_band.Draw("E2 SAME")
    g_data.Draw("P SAME")
    ROOT.gPad.RedrawAxis()                  # ticks on top of the filled stack

    # ---- legend (top-right empty corner) + ATLAS header (top-left).
    leg = ROOT.TLegend(0.62, 0.70, 0.93, 0.92)
    leg.SetTextFont(42)
...
def cmd_massplane(args):
    ...
    atlas_style()
    c = ROOT.TCanvas("c_massplane", "mass-plane exclusion (ROOT mirror)", 800, 750)
    ...
    if d1 > d0:
        ...
        poly.SetFillColorAlpha(ROOT.kGray, 0.45)
        poly.SetLineWidth(0)
        poly.Draw("F SAME")
        keep.append(poly)
        diag = ROOT.TLine(d0, d0, d1, d1)
    ...
    def make_graph(x, y):
        ...
        for a, b in zip(x, y):
            vx.push_back(a)
            vy.push_back(b)
        return ROOT.TGraph(len(x), vx.data(), vy.data())
    ...
    star = ROOT.TMarker(mpar, mlsp, 29)   # 29 = filled five-point star
    star.SetMarkerColor(tcolor(pt_hex))
    star.SetMarkerSize(2.6)
    star.Draw("SAME")
    keep.append(star)
    star_edge = ROOT.TMarker(mpar, mlsp, 30)   # 30 = open star, drawn over for a black outline
    star_edge.SetMarkerColor(ROOT.kBlack)
    ...
    leg = ROOT.TLegend(0.55, 0.74, 0.93, 0.92)
    leg.SetTextFont(42)
    ...
    note = ROOT.TPaveText(0.50, 0.155, 0.93, 0.355, "NDC")
    note.SetFillColor(0)
