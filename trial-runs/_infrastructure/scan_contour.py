...
This is the renderer the corrected mental model points at (workflow/steps/08-scan.md). It does NOT plot
...
Layouts, auto-selected by the scan's shape and the references supplied:
  ...
  * FIG-3 single panel (the HEADLINE artifact, written to <out>__fig3.{png,pdf} whenever BOTH
    ...
    ATLAS compressed-plane convention (LOG Delta m axis -- workflow/checklists/plot-guidelines.md).
...
AXIS SCALES (workflow/checklists/plot-guidelines.md -- the published figure's scales are FACTS,
...
def _basis_guard(scan, what):
    ...
    print(f"  WARNING ({what}): scan.json has NO model_basis -- sigma_ref_fb is then typically the "
          ...
          f"workflow/checklists/scan-and-contour.md Normalization).")
    return False
...
def _exact_grid_lookup(gx, gy, gz, qx, qy, atol=0.05):
    ...
    workflow/checklists/scan-and-contour.md)."""
