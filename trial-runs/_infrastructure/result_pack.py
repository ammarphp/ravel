...
def find_cert(rundir, routine, analysis_id):
    ...
    runs, OR under framework/validation/<id>.json for the run-local certify_axe
    ...
    if repo:
        valdir = os.path.join(repo, "framework", "validation")
        cands = []
        if routine:
            cands.append(os.path.join(valdir, f"{routine}.json"))
...
def _glob(directory, needle):
    ...
    for fn in os.listdir(directory):
        if fn.endswith(".json") and needle and needle.lower() in fn.lower():
...
def rel_to_run(rundir, abspath):
    ...
    relative to the repo root (so framework/validation/<id>.json stays meaningful)."""
    if abspath is None:
        return None
...
def build_figures(rundir, index_path, plot_files):
    ...
    to be set by the plot-criteria check (workflow/checklists/plot-criteria.md).
...
def main(argv=None):
    ...
    if cert_path is None:
        die("cert json not found (looked for outputs/cutflow_cert.json and "
            "framework/validation/<routine|id>.json) -- a run must carry an A*e "
