...
bash "$HERE/export_distribution.sh" "$STAGE"
...
for term in "${GATE_TERMS[@]}"; do
  ...
  if grep -rIlE "$PAT" "$STAGE" 2>/dev/null | head -3 | grep -q .; then
    echo "FAIL: identity term '$term' survives in:"; grep -rIlE "$PAT" "$STAGE" | head -5
    FAILED=1
  fi
done
...
( cd "$STAGE" && python3 framework/check_evidence.py --check --root . >/dev/null ) \
...
if [ "$DO_PUSH" -eq 1 ]; then
  ...
  git add -A
  git commit -q -m "Anonymized distribution snapshot" || echo "(nothing new to commit)"
  git remote remove origin 2>/dev/null || true
  git remote add origin "$ANON_URL"
  if git fetch origin main 2>/dev/null; then
    git push -u origin main --force-with-lease=main:origin/main
  else
    git push -u origin main
  fi
  LOCAL_SHA=$(git rev-parse main); REMOTE_SHA=$(git ls-remote origin main | cut -f1)
  [ "$LOCAL_SHA" = "$REMOTE_SHA" ] || { echo "FAIL: remote != local after push"; exit 6; }
  echo "pushed + remote-verified ($(git rev-parse --short main), author: Anonymous)"
else
