...
Produce showered events (HepMC) for the model. `MG=stages/01-event-generation/build/tools/mg5amcnlo/bin/mg5_aMC`
...
`$CONDA run -n rivet python trial-runs/_infrastructure/resource_census.py --assert-pre-generate
...
1. **Hard process (MadGraph → LHE).** Steering script = the process card with an `output <dir>` line:
   ...
   `checklists/generation-settings.md`. Note σ ± its integration error from the run banner and
   ...
   **Multi-jet merging — required when the SRs count ≥4 jets** ([judgment — proceed-with-flag]: ≥4-jet SRs → merge; EWK/lepton-only/monojet → skip; either way record the choice). A single hard
   ...
   `checklists/merging.md` is the full recipe. In brief: `add process p p > <parents> j` and `… j j`
   ...
   `checklists/merging.md`) before trusting the yields. EWK / lepton-only / monojet searches do
   **not** need merging — record that deliberate choice in `RESULT.md`.
   **Pre-shower guard (always, before any shower time):**
   `$CONDA run -n rivet python trial-runs/_infrastructure/lhe_check.py <procdir>/Events/run01/unweighted_events.lhe.gz --expect-mass <PDG>:<mass> …`
...
2. **Shower (Pythia8 → HepMC3).** Write a Pythia cfg reading the LHE; let the SLHA decay table act:
   ...
   ```
   ```bash
   $CONDA run -n rivet trial-runs/_infrastructure/pythia_shower <cfg> <out.hepmc> <nEvents>
   ```
...
At every step end, run `python3 trial-runs/_infrastructure/workflow_state.py next --rundir <rundir>`:
...
`python3 trial-runs/_infrastructure/workflow_state.py status` lists the detached entries so the agent
can check them itself.

**Next:** `steps/04-analyze.md`
