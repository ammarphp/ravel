...
the target analysis provides (`checklists/choosing-routine.md`). Both apply the analysis selection and
...
objects (the reference paper's chain). Neither is full Geant4 — see `framework/KNOWN-LIMITATIONS.md`.

**Run the detector-fidelity gate first (step 3.5, `checklists/detector-fidelity.md`).** Because the two
...
per routine + detector setup, alongside the cutflow certification (`checklists/validation.md` §B).
...
- **Verify single-weight integrity** (3 checks): the LHE body has no per-event `<rwgt>`/`<wgt>` tags,
  ...
  The LHE-side check is automated: `trial-runs/_infrastructure/lhe_check.py <lhe>` (run pre-shower,
  `steps/03-generate.md`) flags multiweight tags and mixed weight signs — a multiweight LHE in a
...
path does **not** apply (`checklists/complex-analysis.md`; the script detects the case and exits
...
`reference/example-rivet-ewk-path.md`.
...
`reference/native-pipeline.md`. For EwkCompressed2018 / slepton-bino it runs the WHOLE chain
...
→ **LEGACY FALLBACK (unported analyses only): the container path.** `../analysis-simpleanalysis/` runs
...
`reference/native-pipeline.md` §oracle). Use ONLY when the chosen analysis is neither in the
...
(`reference/native-pipeline.md` §porting, CR-005): transcribe on `sa_native_core`, then the
bit-for-bit oracle gate (`cr005_validate.py`). Either path leaves the per-SR yields + (when
