...
step 6 (`checklists/data-acquisition.md` §exclusion-contour), and the two-panel render below. A 1-D Δm
...
are cheap. Before planning a new scan, check `framework/STATUS.md`, `DIRECTORY.md`, and the spec's
...
python3 trial-runs/_infrastructure/validate_parameters.py check  --rundir <scandir> --require-nonempty
...
$CONDA run -n rivet python trial-runs/_infrastructure/scan_orchestrator.py status <scandir>
...
$CONDA run -n rivet python trial-runs/_infrastructure/scan_orchestrator.py assemble <scandir> [--nlo-renorm slepton]
...
#    workflow/checklists/scan-and-contour.md (Normalization).
$CONDA run -n rivet python trial-runs/_infrastructure/scan_orchestrator.py rebase <scandir> --process slepton
...
$CONDA run -n rivet python trial-runs/_infrastructure/scan_contour.py \
...
A ready example is `trial-runs/_infrastructure/specs/slepton_bino_fig3_coarse.json`:
```json
{ "name": "slepton-bino-fig3-coarse", "model": "slepton-bino", "analysis_id": "ins1767649",
  "template_toml": "trial-runs/_infrastructure/templates/slepton_scan_template.toml",
...
armed**: `python3 trial-runs/_infrastructure/preflight_watcher.py --arm --rundir <rundir> --name <watcher>
...
## CHECK-IN
Fulfil the figure contract first (`checklists/figure-contract.md`): the scan contour IS the generated
...
$CONDA run -n rivet python trial-runs/_infrastructure/figure_target.py compose \
  --rundir <rundir> --figure-id "Figure <N>"     # published | this-pipeline side-by-side PNG
```
This final check-in takes the **RESULTS DECK** form (`checklists/check-ins.md`) and is sent only
after the step-9 verification panel (`steps/09-verify.md`), its verdict + findings appended
