...
```bash
$CONDA run -n rivet python trial-runs/_infrastructure/figure_manifest.py \
...
$CONDA run -n rivet python trial-runs/_infrastructure/figure_manifest.py --archetype <A|B|C|D|ESCAPE>
```
...
`checklists/figure-contract.md`). The declared `figure_id` + `hepdata_tables` name the SPECIFIC
...
**Axis SCALES come from the contract, not from heuristics** (see `checklists/plot-guidelines.md`):
...
```bash
$CONDA run -n rivet python trial-runs/_infrastructure/overlay_on_data.py \
...
map is the [judgment] physics descriptor per id (see `checklists/plot-naming.md`). Output lands under
...
```bash
$CONDA run -n rivet python trial-runs/_infrastructure/write_yoda.py \
...
$CONDA run -n recast python trial-runs/_infrastructure/root_figures.py yields \
...
$CONDA run -n recast python trial-runs/_infrastructure/root_figures.py massplane \
...
> (`scan_contour.py`, `steps/08-scan.md`). This §5.4 tool (`mass_plane_overlay.py`) is the **single-point
...
```bash
$CONDA run -n rivet python trial-runs/_infrastructure/mass_plane_overlay.py \
...
Check **every** rendered figure against **`checklists/plot-criteria.md`** (structure: experiment label,
axis titles with units, no chartjunk / no axis-tick-label overlap, legend identifying every series,
uncertainty band, ratio panel; fonts embedded) and **`checklists/plot-guidelines.md`** (construction:
...
$CONDA run -n rivet python trial-runs/_infrastructure/result_pack.py --rundir <rundir> \
  --stat-mode <…> --detector-mode <…>     # → <rundir>/figures.json (schema_version=1)
```
`criteria_pass` (per-figure, NEW vs the INDEX) records the `checklists/plot-criteria.md` verdict for each
...
**THE CAPTION RULE (`checklists/check-ins.md`):** every figure displayed to the physicist — at any
...
or the paper only after it passes `checklists/plot-criteria.md`.

**Next:** `steps/06-acquire-data.md`
