...
Pattern (same idiom as `checklists/merging.md`):
...
| `nevents` | ~10k for a shape/plot demo; **scale it so the weakest SR you intend to certify holds ≥25 raw events** (the tail-SR statistics advisory in `steps/03-generate.md`) |
...
| `use_syst` | **`False` — the pipeline norm** (single nominal weight). `True` bloats the LHE with variation weights and risks a multiweight LHE silently entering the single-weight analysis path; `trial-runs/_infrastructure/lhe_check.py` detects the leak. Set `True` only when a PDF/scale-band study is the explicit goal (and then the whole downstream path must be multiweight-aware) |
...
cards get flagged at audit. (Card recipe: `checklists/model-cards.md`.)
...
`ickkw = 0`, `xqcut = 0`, and regulate jets with `ptj`. MLM merging: `checklists/merging.md`.
...
mandatory pre-shower gate `trial-runs/_infrastructure/lhe_check.py` on every LHE before any shower
time (`steps/03-generate.md`).
