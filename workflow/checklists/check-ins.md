...
(`INITIATE.md`, at the workflow root) promises exactly this sequence.
...
- The declared figure contract: which published figure(s) this run reproduces (`figure_target.py
  show`; `checklists/figure-contract.md`), with the published axis scales recorded at declaration
...
the numbers come from `trial-runs/_infrastructure/cost_preflight.py` (events × points ×
...
`checklists/figure-contract.md` mechanics), captioned per the caption rule — and say explicitly what
...
deliverable the verification panel audits (`checklists/verification-panel.md`, `steps/09-verify.md`);
...
`python3 trial-runs/_infrastructure/validate_run_state.py --edit-guard <path>` — a nonzero exit means
...
3. **Key-numbers table** — every number with its unit AND its artifact-file provenance (the
   ...
   rule (`checklists/verification-panel.md`).
...
7. **Verification-panel verdict** — appended VERBATIM (verdict + itemized findings with dispositions
   + the `verify_pack.py` report), per `steps/09-verify.md`. Never silently fixed.
...
- Gates vs notices: CHECK-INs 1 and 2 are GATES (heavy compute waits for the response); deviation
  ...
  panel verdict (`steps/09-verify.md`).
- Step-boundary self-check (the fallback gate): each check-in sits at a step boundary. When the
  ...
  one before composing the check-in with `trial-runs/_infrastructure/workflow_state.py require
...
- Observer fallback (the L1 twin): `run_state.json` is normally kept current by the PostToolUse
  ...
  input, run `python3 trial-runs/_infrastructure/workflow_state.py record --rundir <rundir> --kind
...
- **Emit + validate (the machine gate, G18):** every check-in is emitted as a machine artifact —
  ...
  `python3 trial-runs/_infrastructure/validate_checkin.py inputs/checkin1.json` (likewise for the
