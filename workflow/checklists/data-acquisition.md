...
| HEPData **complete tables** (the full-table route) | **`hepdata-cli`** (open; bypasses the `/download/` 403) | every numeric table — SR yields, **cutflows/acceptance×eff**, covariance, and (for step 8) the **`exclusion-contour`** boundary + the per-point **`limit`** (σ-UL) grid | `hepdata_fetch.py --tables` classifies each (kind in the manifest); tested, no browser; verifies the download + exits nonzero on a partial fetch. For the step-8 contour-vs-ATLAS deliverable, pick the `exclusion-contour` + `limit` tables for YOUR model (one set per simplified model) — see `steps/06-acquire-data.md` §6.4 → `scan_contour.py --atlas-contour/--atlas-limit` |
...
| HEPData table `/download/` web pages | 403 (Cloudflare) → browser | — | superseded by `hepdata-cli`; browser not needed |
