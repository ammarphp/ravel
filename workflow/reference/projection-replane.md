...
`trial-runs/_infrastructure/spectrum_mix.py` — per target-plane point, TREE-LEVEL neutralino
...
`trial-runs/_infrastructure/replane.py fold` does the DIRECT fold of ONE published limit —
...
```bash
<conda> run -n rivet python trial-runs/_infrastructure/spectrum_mix.py --plane mu:M2 --m1eqm2 \
    --tanb 50 --grid "mu=150:400:50,M2=150:400:50" --json plane.json
<conda> run -n rivet python trial-runs/_infrastructure/replane.py fold --grid plane.json \
...
#3/#7 PASS); step-4 Option D1 (the fold) is live; this spec + `checklists/summary-plot.md` are the
