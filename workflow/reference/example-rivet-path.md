...
A filled-in shape of `WORKFLOW.md` for a jets+E_T^miss search with a Rivet routine. Replace the
...
| 2 model | a colored simplified model (e.g. gaugino/squark pair, parent→jets+LSP); cards per `checklists/model-cards.md` |
| 3 generate | `p p > [parents]` at the routine's √s; **merge** the extra-jet multiplicities if the SRs need ≥4 jets (`checklists/merging.md`) → LHE → Pythia8 → HepMC3 |
