...
(the 0.51% µ₉₅ comparison) back the two claims above — see `EVIDENCE.md`'s
`HEADLINE_native_141_bitforbit` / `HEADLINE_native_mu95_0p51pct` rows for the exact shipped path and
sha256 (the source run directory itself is a dev-only record per `workflow/DISTRIBUTION.md`).
...
σ_tag drift vs the container reference — `framework/CHANGES-REGISTRY.md`), then `{{ecms}}`→6500 GeV,
...
(`templates/slepton_isrslep_generate.mg5`), and `shower.cfg`. A fail-loud guard rejects any unrendered
...
validated for it) → `--backend container` (the mapyde VM path, `analysis-simpleanalysis/`), general
...
- Native RestFrames: `bash trial-runs/_infrastructure/restframes_native_build.sh` → `build/tools/restframes-native/`.
- The RJR resolver: `$CONDA run -n recast bash trial-runs/_infrastructure/rjr_resolve_build.sh`.
...
2. **Transcribe** into `trial-runs/_infrastructure/sa_routines/<name>.py` on `sa_native_core`
...
3. **Register** it in `sa_routines/__init__.py::REGISTRY`; run via
...
5. **ORACLE GATE (mandatory)**: provision the container per-use (`podman machine init/start`
   ...
   `cr005_validate.py --rundir <dir> --config <toml> --routine <Name> --native-txt <txt>`.
