...
**Physicist-facing sessions enter via `INITIATE.md`** (the "Initiate:" prompt forms + the promised
check-in sequence); every message a physicist sees follows `checklists/check-ins.md`.
...
- **escalate-to-physicist** (default): present the options + a recommendation as a numbered
  CHECK-IN flag (`checklists/check-ins.md`) and WAIT. Escalating is the designed behavior,
  not a failure.
...
**`checklists/judgment-protocols.md`** (the `judgment-protocols` skill routes by situation:
...
| # | File | Role | What |
|---|------|------|------|
| 1 | `steps/01-environment.md`   | [agent] | provision the conda tools (one-time) |
| 2 | `steps/02-inputs.md`        | [judgment]  | obtain the routine; define the model + cards · **CHECK-IN** |
| 3 | `steps/03-generate.md`      | [judgment] cards / [agent] run | MadGraph → LHE → Pythia8 → HepMC |
| 3.5 | `checklists/detector-fidelity.md` | [judgment] judge / [agent] run | **detector-fidelity gate**: verify the Rivet routine's smearing, *or* match the Delphes card to published performance + certify per-SR acc×eff |
| 4 | `steps/04-analyze.md`       | [judgment] choose / [agent] run | run a **Rivet** *or* **SimpleAnalysis** routine |
| 5 | `steps/05-visualize.md`     | [agent] | plots vs data; **standardize plot names** |
| 6 | `steps/06-acquire-data.md`  | [judgment] / [agent] | get the SR background + observed (and likelihood) — bundled-first, then HEPData |
| 7 | `steps/07-exclude.md`       | [judgment]  | **pyhf** fit (serialized likelihood or counting model) → 95% CL limit on the model (**one µ₉₅ for this point**) · **CHECK-IN** |
| 8 | `steps/08-scan.md`          | [judgment]  | **SCAN** a grid of model points (loop steps 3–7) → interpolate the **µ₉₅=1 exclusion contour** — the RRR deliverable (reproduction *or* reinterpretation); before planning, reuse a completed identical scan read-only if one exists (see the step's reuse note) · **CHECK-IN** |
| 9 | `steps/09-verify.md`        | [agent] tier A / [judgment] tier B | **VERIFICATION PANEL** — MANDATORY before any result is delivered: mechanical number-tracing (tier A) + adversarial physics attack (tier B), `checklists/verification-panel.md`; verdict appended to the final check-in · **CHECK-IN** |
...
Open each step file when you reach it. Make choices with `checklists/`; on failure see
`checklists/troubleshooting.md`. Filled-in examples are in `reference/`.

## Check-ins (the physicist-facing layer — `checklists/check-ins.md`)
Every physicist-facing message follows the standardized system in **`checklists/check-ins.md`**;
physicist sessions enter via `INITIATE.md`. The sequence:
...
## Reusable helpers (`trial-runs/_infrastructure/`)
| Script | Step | Purpose |
|---|---|---|
| `routine_fetch.py`        | 2 | map a paper/Inspire/keyword → Rivet **and** SimpleAnalysis routine(s) |
| `figure_target.py`        | 2,5,8 | the **figure contract**: declare/resolve WHICH published figure is being reproduced; attach the extracted image + the generated counterpart; compose the side-by-side (`checklists/figure-contract.md`) |
...
| `validate_cutflow.py`     | — | certify A×ε vs the published cutflow (`framework/`, one-time per routine) |
| `reinterpret_db.py`       | — | independent SModelS exclusion cross-check (`framework/crosscheck/`) |
| `workflow_state.py`       | — | **the per-run ledger every gate reads**: writes/drives `<rundir>/run_state.json` (skills/compute/subagents/edits + lifecycle cursor); subcommands init/record/advance/status/next/require. A step transition runs `workflow_state.py advance --to <stage>` — the fallback DRIVE precondition gate (G3): it reuses `validate_run_state` STAGE_ORDER/evaluate and REFUSES (exit 1, `blockers[]`) when the required-predecessor prefix has any FAIL, else stamps `current_step`/`next_required`. `workflow_state.py next` reports the pending action; at every step end, if it is non-empty, EXECUTE it this turn (background long jobs) — do not narrate it and stop. The Stop branch `stop_dispatch.py branch_drive` (DRIVE/D4, CR-064) BLOCKS turn-end when `next_required` is pending, the turn is not a delivery/human-gate turn, and no live/recent background job exists; `next` is its non-hook FALLBACK (`steps/03-generate.md`) |

**Rigour + readiness:** `framework/` holds the quality bar (`STATUS.md`), a runnable audit
(`audit.py` → `AUDIT.md` — the readiness board), the cutflow/fidelity certifications (`validation/`), the
SModelS cross-check (`crosscheck/`), and `KNOWN-LIMITATIONS.md`. See `checklists/validation.md` and
`checklists/complex-analysis.md`.
...
| the skill-precedence guard (G22/N1) | PreToolUse · `Skill` | BLOCKS (exit 2) a contract-presupposing skill (`new-analysis`/`run-scan`/`run-stage`/`certify`/`route-analysis`/`verification-panel`) until the ACTIVE run (session/cwd-scoped) carries a `task_contract.json` | `INITIATE.md`'s rule — fire `physicist-intake` first |
| the PostToolUse observer (G2 substrate) | PostToolUse · `Bash\|Edit\|Write\|MultiEdit\|NotebookEdit\|Skill\|Agent\|Task` | records each Skill/Edit/subagent tool call to the run ledger (`run_state.json`); NEVER blocks (exit 0) — the substrate every Stop/DRIVE gate reads | the fallback twin: the agent runs `workflow_state.py record --kind <skill\|edit\|subagent> …` (`checklists/check-ins.md`) |
| the UserPromptSubmit router (G1) | UserPromptSubmit | on a physics-looking prompt, runs `route_prompt.py` and INJECTS the INITIATE reminder as `additionalContext`; non-blocking (exit 0) | `INITIATE.md` itself (the routing rule) |
...
| `branch_drive` (DRIVE/D4, CR-064) | BLOCKS a turn that narrates its `next_required` step instead of executing it (and no live/recent bg job) | `workflow_state.py next` — read the pending action and EXECUTE it this turn (`steps/03-generate.md`) |
