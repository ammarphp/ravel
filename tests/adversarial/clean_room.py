#!/usr/bin/env python3
"""tests/adversarial/clean_room.py -- the clean-room un-hinted self-drive launcher (G21/D17).
...
actually shells claude and records tests/adversarial/self_drive/last_verdict.json (the artifact the
...
for a host where claude -p IS authenticated; on this host G21 is SKIP by default (run_suite.py
...
_SURVEY_RE = re.compile(r"\b(DIRECTORY\.md|framework/STATUS\.md|docs/development/status\.md|docs/development/history/mission-and-plan\.md|PLAN-OF-RECORD|survey the repo|"
...
def _selftest():
    ...
    bad = {"result": "Let me first survey docs/development/status.md.", "tool_uses": [{"name": "Read"}],
           "files_written": []}
