"""scripts/audit.py must be read-only by default (Task 1.1, write-suppression only).

audit.py used to write docs/development/audit.md unconditionally as a side effect of running it —
...
AUDIT_MD = REPO / "docs" / "development" / "audit.md"
...
def test_check_writes_nothing():
    ...
    assert after == before, "scripts/audit.py (no args) must not modify docs/development/audit.md"
