...
def test_guard_blocks_unapproved_smoke_gen(tmp_path):
    ...
    r = _guard("bash native/scripts/run-pipeline-native.sh trial-runs/run1 cfg.toml",
               proj, rd, session="S1")
...
def test_guard_rejects_malformed_contract_even_with_approval(tmp_path, raw):
    ...
    result = _guard("bash native/scripts/run-pipeline-native.sh trial-runs/run1 cfg.toml", proj, rd, session="S1")
...
def test_guard_fails_closed_when_validator_is_missing_in_a_physics_run(tmp_path):
    proj, rd = _skel(tmp_path, approved=True)
    (proj / "src" / "ravel" / "validation" / "validate_task_contract.py").unlink()
    result = _guard("bash native/scripts/run-pipeline-native.sh trial-runs/run1 cfg.toml", proj, rd, session="S1")
    assert result.returncode == 2
...
def test_source_changes_invalidate_approval_until_rerecorded(tmp_path, source):
    ...
    command = "bash native/scripts/run-pipeline-native.sh trial-runs/run1 cfg.toml"
...
def test_planned_scan_can_have_a_bound_smoke_approval(tmp_path):
    ...
    assert result.returncode == 0, result.stderr
    guarded = _guard("bash native/scripts/run-pipeline-native.sh trial-runs/run1 cfg.toml", proj, rd, session="S1")
...
@pytest.mark.parametrize("command", [
    "python3 src/ravel/workflow/scan_orchestrator.py launch trial-runs/run1 --go",
    "python3 src/ravel/workflow/scan_babysitter.py trial-runs/run1 --once",
    "python3 src/ravel/workflow/scan_orchestrator.py launch trial-runs/run1 --go; echo --help",
    "python3 src/ravel/workflow/scan_orchestrator.py launch trial-runs/run1 --go\necho --help",
    "python3 src/ravel/workflow/scan_orchestrator.py launch trial-runs/run1 --go # --help",
    "python3 src/ravel/workflow/scan_orchestrator.py launch trial-runs/run1 --go # note\necho --help",
    "python3 src/ravel/workflow/scan_orchestrator.py launch trial-runs/run1 " + chr(92) + "\n --go; echo --help",
])
...
@pytest.mark.parametrize("command", [
    "python3 src/ravel/workflow/scan_orchestrator.py launch trial-runs/run1",
    "python3 src/ravel/workflow/scan_orchestrator.py status trial-runs/run1",
    "python3 src/ravel/workflow/scan_orchestrator.py launch --help",
    "python3 src/ravel/workflow/scan_babysitter.py --help",
    "echo python3 src/ravel/workflow/scan_orchestrator.py launch trial-runs/run1 --go",
])
