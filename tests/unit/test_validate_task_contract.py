...
spec = importlib.util.spec_from_file_location("task_contract_under_test", REPO / "src/ravel/validation/validate_task_contract.py")
...
def test_real_router_and_cost_producer_contracts_validate():
    ...
    for prompt in ("Survey ATLAS searches for a slepton.",
        ...
        result = subprocess.run([sys.executable, str(REPO / "src/ravel/workflow/route_prompt.py"), "--prompt", prompt, "--print"], capture_output=True, text=True)
...
def test_archival_cli_does_not_call_it_a_valid_live_contract(tmp_path):
    ...
    strict = subprocess.run([sys.executable, str(REPO / "src/ravel/validation/validate_task_contract.py"), str(path)], capture_output=True, text=True)
    archive = subprocess.run([sys.executable, str(REPO / "src/ravel/validation/validate_task_contract.py"), "--legacy", str(path)], capture_output=True, text=True)
...
def test_cli_rejects_ambiguous_or_nonstandard_json(tmp_path, raw, needle):
    ...
    path.write_text(raw)
    result = subprocess.run([sys.executable, str(REPO / "src/ravel/validation/validate_task_contract.py"), str(path)], capture_output=True, text=True)
...
def test_schema_is_machine_readable_and_carries_compatibility_policy():
    result = subprocess.run([sys.executable, str(REPO / "src/ravel/validation/validate_task_contract.py"), "--schema"], capture_output=True, text=True)
...
def _lifecycle():
    spec = importlib.util.spec_from_file_location("contract_lifecycle_under_test", REPO / "src/ravel/validation/validate_run_state.py")
...
def test_advancing_to_first_stage_cannot_bypass_schema(tmp_path):
    ...
    cmd = [sys.executable, str(REPO / "src/ravel/workflow/workflow_state.py")]
