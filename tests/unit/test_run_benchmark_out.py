"""src/ravel/validation/benchmark.py must not clobber the tracked results.json by default
(Task 1.2, write-suppression only). run_benchmark.py used to default --out to the TRACKED
benchmarks/results.json and overwrite it on EVERY run (even --fast) — a verification run
dirtied a committed baseline. This pins the fix: the default --out target is a gitignored scratch
path (benchmarks/.work/results.latest.json); refreshing the tracked baseline requires the
...
Argparse introspection only: no pyhf/conda or physics run. The tests work from an unrelated cwd.
"""
import importlib.util
from pathlib import Path

REPO = Path(__file__).resolve().parents[2]
RUN_BENCHMARK_PY = REPO / "src/ravel/validation/benchmark.py"
import sys
sys.path.insert(0, str(REPO / "src"))
from ravel.resources import resource_root
TRACKED_RESULTS = resource_root() / "benchmarks/results.json"
...
def _load_module():
    ...
    spec = importlib.util.spec_from_file_location("ravel.validation.benchmark", RUN_BENCHMARK_PY)
...
def test_explicit_out_still_works_even_without_update_baseline():
    ...
    'an explicit --out benchmarks/results.json must also still work')."""
    mod = _load_module()
    parser = mod.build_parser()
...
def test_no_update_baseline_no_explicit_out_keeps_scratch_default():
    ...
    args = parser.parse_args(["--fast"])
    resolved = mod.resolve_out(args)
    assert Path(resolved) == REPO / "benchmarks" / ".work" / "results.latest.json"
