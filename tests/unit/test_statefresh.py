...
DIRECTORY.md, docs/reference/scope.md, the step-4/effmap/projection-replane/decision-shape-fit docs)
must reconcile to `benchmarks/capabilities.json`: (1) the gen_status.py generated blocks are
...
CAS_PATH = REPO / "src" / "ravel" / "validation" / "check_agent_surface.py"
...
def test_clean_repo_passes():
    ...
    result = subprocess.run([sys.executable, str(REPO / "scripts/run.py"), "ravel.validation.check_agent_surface"], cwd=REPO,
...
def test_catches_stale_claim(tmp_path):
    ...
    guarded_file = tmp_path / "docs" / "reference" / "limitations.md"
