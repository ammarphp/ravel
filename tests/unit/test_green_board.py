# tests/unit/test_green_board.py
"""scripts/green_board.py -- the aggregate make-green board (Task 6.7). Fabricated-rung fixtures
...
GB = REPO / "scripts" / "green_board.py"
...
def test_real_fast_rung_executes_from_repo_root(tmp_path, capsys):
    ...
    rj = _rungs(tmp_path, [["contract",
                            "python3 scripts/run.py ravel.validation.validate_task_contract --selftest",
                            False]])
    rc = mod.main(["--rungs-json", str(rj)])
    out = capsys.readouterr().out
    assert rc == 0, out
