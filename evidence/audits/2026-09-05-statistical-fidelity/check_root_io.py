...
def verify():
    ...
    workspace = {
        ...
        'measurements': [{'name': 'Measurement', 'config': {
            'poi': 'background_scale', 'parameters': []}}],
    }
    ...
    with tempfile.TemporaryDirectory(prefix='ravel-root-io-') as temporary:
        ...
        background = directory / 'MonoJet-background.json'
        background.write_text(json.dumps(workspace, indent=2) + '\n')
        before = background.read_bytes()

        def run_case(name, branches, *, lumi=3, compressed=False, expected_error=None):
            ...
            command = [sys.executable, str(engine), '-i', str(ntuple), '-b', str(background),
                       '-o', str(output), '-n', 'signal', '-l', str(lumi)]
            if compressed:
                command.append('-c')
            result = subprocess.run(command, capture_output=True, text=True, timeout=60,
                                    env={**os.environ, 'PYTHONDONTWRITEBYTECODE': '1'})
            record = {'name': name, 'exit_code': result.returncode, 'patch_exists': output.exists(),
                      'input_root_sha256': hashlib.sha256(ntuple.read_bytes()).hexdigest()}
            if expected_error is not None:
                ...
                require(result.returncode == 0 and output.exists(), f'{name}: converter did not produce a patch')
                patched = jsonpatch.apply_patch(workspace, json.loads(output.read_text()), in_place=False)
                model = pyhf.Workspace(patched).model(poi_name='mu_SIG')
        ...
        masked = run_case('signed-masked', {'Z': [2., -.5, 9.], 'A': [3., -1., 8.],
                                           'ismm': [1., 1., 0.]}, lumi=1, compressed=True)
    ...
    return {
        ...
        'background_unchanged': True, 'cases': reports,
        'scope': 'Actual tiny ROOT TTree integration, including real Awkward-array reads and pyhf/JSON Patch validation. '
                 ...
                 'are rejected. This is input/output correctness, not statistical coverage or a finite-limit claim.',
    }
